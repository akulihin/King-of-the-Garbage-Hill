...
{
    public sealed record OmniManIntelligenceDialogue(
        string Question,
        string EnemyReply,
        string OmniReply);

    public static readonly IReadOnlyList<string> OmniManIntelligenceCheckPhrases = new List<string>
    {
        "Что было первым, курица или яйцо?",
        "Что тяжелее, килограмм ваты или килограмм гвоздей?",
        "Что тяжелее, килограмм ваты или мой кулак?",
    ...
    public static readonly IReadOnlyList<OmniManIntelligenceDialogue> OmniManIntelligenceSuccessDialogues =
        new List<OmniManIntelligenceDialogue>
        {
            new(
                "Что было первым, курица или яйцо?",
                "Не знаю, наверное курица...",
                "Вот именно! А значит батя прав!"),
            new(
                "Зачем присоединяться ко мне и жить, когда можно умирать страшной смертью?",
                "Наверное... лучше не умирать?",
                "Отлично, сын. Вместе, нас ждут великие дела!"),
            new(
                "Я убиваю этих людей, чтобы убедить **тебя**. Следовательно, это **ТЫ** убиваешь людей своими отказами!",
                "Не вижу противоречий.",
                "Потому что их нет!"),
            new(
                "Не учи отца ебаться, иначе тебя бы тут не было.",
                "А вот с этим не поспорить.",
                "Батя прав по определению!"),
            new(
                "...",
                "Буль...",
                "Что это за планета такая? Что и Интеллектом? Вы что, все Братишки тут?")
        };

    public static readonly IReadOnlyList<string> OmniManGuardiansPhrases = new List<string>
    {
        "\"(ник врага) оказался Стражем Земли! Поэтому он пропустит свой следующий ход.\"",
        "\"(ник врага) пропустит свой следующий ход. Был коллега, стал калека! Got it?\"",
        "\"(ник врага) пропустит свой следующий ход. Нолан нанес ему визит. Больше (ник врага) не сможет ходить.\"",
        "\"(ник врага) пропустит всего-то один ход, подумаешь. Это даже не тысяча лет в плену Вилтрумайтов.\"",
        "\"(ник врага) пропустит свой следующий ход. Он бы не прошел церемонию взросления на меой планете...\"",
        "\"(ник врага) пропустит свой следующий ход. И стоило оно того, Марк?!\"",
        "\"(ник врага) пропустит свой следующий ход. Они для нас лишь насекомые.\"",
        "\"(ник врага) пропустит свой следующий ход. Ничтожество.\"",
        "\"(ник врага) пропустит свой следующий ход... хотя это уже конец, всем всё равно.\""
    };

    public static readonly IReadOnlyList<string> OmniManParticlePhrases = new List<string>
    {
        "\"Ты жалок.\"",
        "\"Ничтожество.\"",
        "\"Слишком медленно, слишком слабо, слишком скучно.\"",
        "\"Вы насекомые.\"",
        "\"Жалкая имитация.\"",
        "\"Это и есть ваша... Наука?\"",
        "\"Люди не больше чем питомцы.\"",
        "\"У людей есть поговорка: \"Не встречай своих героев\". Они правы.\"",
        "\"Ты правда думал, что у тебя есть шанс? Это даже не самоуверенность. Это глупость.\"",
        "\"Ваша планета кипит волей к жизни. Пока.\"",
        "\"Вы даже не представляете, с чем имеете дело.\"",
        "\"Это не бой. Это урок.\"",
    ...
    public static readonly IReadOnlyList<string> HomelanderDropPhrases = new List<string>
    {
        ...
        "\"Я думал мы команда... Семерка! Нет никакой семерки. Есть только я.\"",
        "\"Я собрал вас! Без меня ты был бы никто! И вот так ты мне отплатил?\"",
        "\"Никто не будет скучать по нему. Серьёзно, назови хоть одного человека.\"",
        "\"Он сам напросился. Серьезно.\"",
        "\"Ты меня вынудил!\"",
        "\"Я считал тебя своим другом... А ты меня предал!!!\"",
        "\"Я... Один.\"",
        "\"Вы все сегодня сговорились взбесить меня?!\"",
        "\"Вы что, хотите еще один Рейс 37 ?\""
    };

    public static readonly IReadOnlyList<string> HomelanderRevealPhrases = new List<string>
    {
        "(ник врага) разузнал вашу личность. Патриот: \"Ты знаешь, кто я? Нет, ты не знаешь. Ты знаешь маску. Ты не знаешь меня.\"",
        "(ник врага) разузнал вашу личность. Патриот: \"Я и не помню когда в последний раз ходил без костюма...\"",
        "(ник врага) разузнал вашу личность. Патриот: \"Не называй это имя! Теперь я - Патриот!\""
    };

    public static readonly IReadOnlyList<string> HomelanderEnemyVoughtPhrases = new List<string>
    {
        "\"Рейтинги (ник врага) будут уничтожены!\"",
        "\"Мы пустим эфир о том, что (ник врага) ебал козу!\"",
        "\"Мы отправили Нуара к (ник врага), пусть он там разберется.\"",
        "\"Не волнуйтесь, этот (ник врага) будет устранен.\"",
        "\"Наши тролли закидают (ник врага) какашками в комментариях.\"",
        "\"(ник врага) будет разоблачен в интернете на канале Сплетни Vought. Подписывайтесь!\" ",
        "\"Зато рейтинг (ник врага) упал на 1%!\"",
        "\"По данным наших аналитиков, успех (ник врага) — случайность.\""
    };

    public static readonly IReadOnlyList<string> HomelanderOwnVoughtPhrases = new List<string>
    {
        "\"Общественное мнение — это просто товар. И я владею контрольным пакетом.\"",
        "\"Ты правда думал, что люди поверят тебе, а не мне? Посмотри на рейтинги, приятель.\"",
        "\"Очередной героический поступок от вашего Патриота.\"",
        "\"Я всех вас спас. Опять. Можете не благодарить.\"",
        "\"Да-да, я победил его. Это **Я** сделал.\"",
        "\"Да, я победил его. Это был... Супер...терор.. СуперЗлодей!\""
    };

    public static readonly IReadOnlyList<string> HomelanderProtectionPhrases = new List<string>
    {
        "\"Я вам не какой-то там человек! Я... Бог.\"",
        "\"Жалким людишкам меня не понять.\"",
        "\"Я ваша миссия! Отъебитесь блять уже от меня!\"",
        "\"Я могу сделать всё, что захочу. Абсолютно всё. И никто меня не остановит. *яростно дрочит*\"",
        "\"Ты серьёзно? Ты думал, что это сработает? Я — Хоумлендер. Твердыня. Патриот!\""
    };

    public static readonly IReadOnlyList<string> HomelanderTheBoysDropPhrases = new List<string>
    {
        "\"Что... что вы со мной сделали?\"",
        "\"ВИЛЬЯМ БУЧЕР!\"",
        "\"Ты хочешь отнять у меня то, что я любил!\""
    };
