...
King of the Garbage Hill — turn-based 6-player tactical game, 42 character definitions (including secret/transform-only entries). Hybrid **Discord bot + ASP.NET Core web server** (single process, .NET 10, Lamar DI, Discord.Net 3.20), **Vue 3 + TypeScript** client via SignalR. Mixed Russian/English; **Russian passive/character names are load-bearing string identifiers** — never paraphrase or "fix" them.
...
Two project hooks enforce this automatically (`.Codex/settings.json`): after any edit to passive-bearing files the passive audit re-runs and reports NEW warnings; on stop, a reminder fires if game code changed without a docs update. Don't be surprised by their output — act on it.
...
- `Status` is the SAME instance on `GameCharacter`/fight copies, but `Justice` is deep-copied. A round-wide immutable `RoundFightCharacter` template produces a fresh `FightCharacter` for every fight, so all class/stat/Justice reads use that frozen clone; lasting Justice changes go explicitly to `GameCharacter`. A new List/Dictionary field on `CharacterClass` needs a line in `DeepCopy()`.
- Real Psyche-loss events go through `player.MinusPsyche(game, -N, "PassiveName")`: all five immunities always apply and the global «игрок психанул» log is written by default. A passive with its own public text passes `suppressDefaultGlobalLog: true`; this suppresses only the default text, never immunities. Raw negative `AddPsyche`/`SetPsyche` is reserved for documented self-costs, rollback of temporary stats and structural transformations.
...
- **Never invent unsolicited player-, enemy- or global-facing copy**, especially automatic receipts sent to an effect owner's enemies. Add only text supplied or explicitly requested by the designer. If an enemy-forced Skip genuinely needs feedback, reuse the exact neutral default `Тебя усыпили...`; otherwise ask for authored wording instead of improvising it.
