...
<template>
  <div class="fa-card" :class="{
    'fa-shake': fightShake,
    'fa-result-win': fightResult === 'win',
    'fa-result-loss': fightResult === 'loss',
    'storm-result-hit': fight.stormAppeared && showR1Result && !skippedToEnd,
  }">
    <!-- Block/Skip -->
    ...
    <template v-else>
      ...
      <div
        v-if="fight.stormAppeared && showR1Result"
        class="storm-intervention"
        :class="{
          'storm-helped-us': fight.stormWeighingDelta * sign > 0,
          'storm-hurt-us': fight.stormWeighingDelta * sign < 0,
          'storm-instant': skippedToEnd,
        }"
      ...
      >
        ...
        <div class="storm-receipt">
          ...
          <strong v-if="fight.stormWeighingDelta !== 0" class="storm-delta">
            {{ fight.stormWeighingDelta * sign > 0 ? '+' : '' }}{{ fight.stormWeighingDelta * sign }}
          </strong>
...
.fa-id-left, .fa-id-right { display: flex; align-items: center; gap: 6px; flex-shrink: 0; }
...
.fa-ava-sm { width: 60px; height: 60px; border-radius: 50%; object-fit: cover; border: 2px solid var(--border-subtle); flex-shrink: 0; }
.fa-id-info { display: flex; flex-direction: column; line-height: 1.2; }
.fa-id-name { font-size: 12px; font-weight: 700; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100px; }
.fa-id-char { font-size: 10px; color: var(--text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100px; }
...
.fa-card.storm-result-hit { animation: storm-arena-jolt 0.45s ease-out; }
.storm-intervention {
  position: absolute;
  inset: 0;
  z-index: 24;
...
@media (prefers-reduced-motion: reduce) {
  .fa-card.storm-result-hit { animation: none; }
