...
Two independent callers, both model `claude-haiku-4-5-20251001` (`GameStoryService.cs:29-31` `ClaudeHaikuService.cs:20`), key from `AnthropicApiKey` (`Config.cs:28`):

- **`GameStoryService`** (`GameStoryService.cs:21-42`; limits `GameStoryService.cs:29-32`): fire-and-forget post-game narrative, max 1800 tokens **per language request**. Triggered from the finish path (§6); skips missing key and **bot-only games** (`GameStoryService.cs:49-56`). Its snapshot uses replay v2's captured playable rounds and structured fight pairs, retains explicit `HandleLastRound` suffixes only as `final-settlement`, and falls back to terminal `RoundNo - 1` only if no replay capture exists (`GameStoryService.cs:168-314`). The prompt supplies the strongest repeated rivalries and selects a deterministic game-ID-seeded frame, structure and chaos device from 216 combinations; separate RU and EN instruction sets request plain story prose without output wrapper tags (`GameStoryService.cs:335-526`). Russian and English calls run concurrently and fail independently; an empty response retries only that language once. Russian is always requested. English is default-off: set `"GameStoryEnglishEnabled": true` in `DataBase/config.json` to request it; false or an omitted setting skips that API call (`GameStoryService.cs:65-141`; `Config.cs:27-29`). If at least one call succeeds, both locale containers are stored, pushed as eventType `GameStory`, and backfilled into the replay; a missing language receives a localized unavailable message instead of suppressing the successful story or opening a blank tab (`GameStoryService.cs:77-95,630-640`; `GameHub.cs:134-137`; `GameNotificationService.cs:70-74`). Raw API call: `CallClaudeApi` (`GameStoryService.cs:656-690`).
