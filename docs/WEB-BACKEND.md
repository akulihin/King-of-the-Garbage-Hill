...
All account-economy operations synchronize on the `DiscordAccountClass` instance: the finish path settles account/history/mastery/ZBS/quest/top-two-box/achievement/statistics together; Battleship settles its W/L, streak and first-win ZBS while holding the game then account monitors; Daily Quest initialization/reroll, loot debit+roll+credit, reward reads/acknowledgements, paid draft picks and Discord-store changes use that same account monitor (`CheckIfReady.cs:722-796`; `BattleshipService.cs:35-119`; `QuestClass.cs:358-508,638-723`; `GameHub.cs:649-725,811-1018`; `WebGameService.cs:368-463`; `GameReactions.cs:512-610`; `StoreReactions.cs:196-515`). This prevents simultaneous reward, reroll, pick or shop actions from losing a balance/box/progress update.
