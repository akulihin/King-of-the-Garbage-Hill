...
The English catalog is copied to the deployed DataBase output and loaded lazily by `GameLocalization`; Vue imports that same file rather than maintaining a second translation table. Dynamic templates run before longest-first exact/term replacement; character-phrase markers continue through the rest of their multi-line log block and use the shared passive-aware fallback catalog (`GameLocalization.cs:89-142`; catalog `localization.en.json:936-1060`). This also normalizes older mixed snapshots whose passive title was translated before their phrase. The audit requires English content/display coverage for all characters/passives plus both display and fallback coverage for all 132 `PhraseClass` identifiers (`tools/audit-localization.sh:13-78`). Game stories request paired native RU/EN tagged adaptations (max 1800 tokens) and store both in one replay-safe HTML value (`GameStoryService.cs:26-29,68-77,172-196`). Geralt's hint request receives the owner's locale and uses matching static dictionaries on failure (`ClaudeHaikuService.cs:37-65`, `CP:4657-4688`). Full contract: [LOCALIZATION.md](LOCALIZATION.md).
