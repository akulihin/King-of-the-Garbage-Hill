...
**Character-specific actions:** `DarksciChoice(gameId, isStable)` `GameHub.cs:369-378`; `YoungGleb(gameId)` `GameHub.cs:380-389`; `DopaChoice(gameId, tactic)` `GameHub.cs:391-400`; `DeathNoteWrite(gameId, targetPlayerId, characterName)` (Kira) `GameHub.cs:404-413`; `ShinigamiEyes(gameId)` `GameHub.cs:415-424`; Salldorum's `ActivateShen(gameId, position)`, `DeactivateShen(gameId)`, `RewriteHistory(gameId, roundNumber)` `GameHub.cs:428-459`.

**Meta (account-level, no game):** `RequestQuests()` → `QuestState` (`GameHub.cs:543-574`); `OpenLootBox()` → decrements `PendingLootBoxes`, rolls loot, replies `LootBoxOpened` (`GameHub.cs:578-604`); `RequestAchievements()` → full board with secret achievements masked as `"???"` until unlocked (`GameHub.cs:608-650`); `ClearNewAchievements()` (`GameHub.cs:652-660`).
