...
| DooM Guy state | owner-only widget contains active/options/nests/BFG/Chainsaw plus live module progress: protected Rune grants, extermination count, Shield use/block count, Mission challenge success/failure and Chainsaw consumption. Let's Roll viewers receive empty character catalogs so predictions cannot be reconstructed client-side | `GameStateMapper.cs` `DoomGuyStateDto` mapping; DTO `DoomGuyStateDto` |
