...
The general English catalog is copied to deployed `DataBase` and loaded lazily by `GameLocalization`; Vue imports it for ordinary bidirectional presentation. Longest-first fragments include markup-split and partially projected replay forms, such as `**обычных** очков`, `Class: +2 Cкилла`, Justice encouragements, troll outcomes and username-prefixed winner lines. Dynamic rules are idempotent across canonical and partially translated snapshots: Auto Move, Darksci's tilt sentence and nemesis defeat connectors finish the sentence while leaving arbitrary player names untouched (`GameLocalization.cs:34-49`). Character flavor uses the separate backend-owned `phrases.en.json`: every one of its 235 groups carries canonical/English titles and ordered Russian/English pairs for all 796 variants. Startup validates group/count parity and exact Russian source parity by index (`PhraseLocalization.cs:19-69,165-198`). A selected flavor line is stored as a compact bilingual payload, while legacy replay markers are resolved from those same contextual pairs (`PhraseLocalization.cs:72-163`). Dynamic Witcher-senses output follows the same payload boundary: Haiku produces both variants or the backend supplies the paired static fallback, then `PhraseV2` carries both through Discord, web and replay projection (`CharacterPassives.cs:4678-4712`). `tools/audit-localization.sh` also rejects a regression to the old single-language generator. Full contract: [LOCALIZATION.md](LOCALIZATION.md).
