...
Two independent callers, both model `claude-haiku-4-5-20251001` (`GameStoryService.cs:29` `ClaudeHaikuService.cs:20`), key from `AnthropicApiKey` (`Config.cs:28`):

- **`GameStoryService`** (`GameStoryService.cs:21-41`; limits `GameStoryService.cs:28-31`): fire-and-forget post-game narrative, max 1800 tokens. Triggered from the finish path (§6); skips missing key and **bot-only games** (`GameStoryService.cs:47-54`). Its snapshot uses replay v2's captured playable rounds and structured fight pairs, retains explicit `HandleLastRound` suffixes only as `final-settlement`, and falls back to terminal `RoundNo - 1` only if no replay capture exists (`GameStoryService.cs:129-275`). The prompt supplies the strongest repeated rivalries and selects a deterministic game-ID-seeded frame, structure and chaos device from 216 combinations; it asks for multi-character action/reaction chains while forbidding invented gameplay (`GameStoryService.cs:296-521`). The response must contain non-empty `<ru>` and `<en>` blocks parsed by the shared generated-text contract; malformed output gets one format-constrained retry and is then discarded rather than published as a single-language replay artifact (`GameStoryService.cs:59-101`; `BilingualGeneratedText.cs:6-32`). Valid blocks are formatted separately, stored together, pushed as eventType `GameStory`, and backfilled into the replay (`GameStoryService.cs:82-94`; `GameHub.cs:134-137`; `GameNotificationService.cs:70-74`). Raw API call: `CallClaudeApi` (`GameStoryService.cs:582-616`).
