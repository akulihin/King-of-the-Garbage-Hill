...
| `BattleshipLobby`, `BattleshipStats`, `BattleshipGameCreated`, `BattleshipGameJoined`, `BattleshipState`, `BattleshipEvent` (eventType `ShotResult`), `ShipCatalog` | lobby list; caller-only persistent W/L/streak/first-win/ZBS snapshot; personalized game state; shot event; catalog | battleship flow | `GameHub.cs:1172-1229` `GameHub.cs:1247-1259` `GameHub.cs:1364-1431` `GameHub.cs:1529-1564` |
...
- Blackjack tables and live battleship games are in-memory only—restarts drop active matches. Battleship account W/L, streak and earned ZBS persist with the account; its cleanup timer sweeps every 5 minutes and removes matches idle for 30 minutes (`BattleshipService.cs:23-32,1385-1395`).
