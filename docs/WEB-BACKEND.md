...
The general English catalog is copied to deployed `DataBase` and loaded lazily by `GameLocalization`; Vue imports it for ordinary bidirectional presentation. Character flavor uses the separate backend-owned `phrases.en.json`: startup pairs all 235 `PhraseClass` fields and validates all 796 indices (`PhraseLocalization.cs:19-62`). A selected flavor line is stored as a compact bilingual payload, so live web projection and replay snapshots retain the exact RU and EN rendering; media DTOs expose paired fields (`GameStateMapper.cs:1088-1106`). Discord resolves payloads before sorting and stories resolve them before prompt cleanup. Legacy `|>Phrase<|` records retain passive-aware fallback support. `tools/audit-localization.sh` runs the exhaustive phrase parity audit. Full contract: [LOCALIZATION.md](LOCALIZATION.md).
