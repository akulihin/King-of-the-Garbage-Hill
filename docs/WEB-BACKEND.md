...
- **`GameStoryService`** (`GameStoryService.cs:19-40`; limits `GameStoryService.cs:26-29`): fire-and-forget post-game narrative, max 1800 tokens. Triggered from the finish path (§6); skips missing key and **bot-only games** (`GameStoryService.cs:46-53`). The response must contain non-empty `<ru>` and `<en>` blocks parsed by the shared generated-text contract; malformed output gets one format-constrained retry and is then discarded rather than published as a single-language replay artifact (`GameStoryService.cs:58-100`; `BilingualGeneratedText.cs:6-32`). Valid blocks are formatted separately, stored together, pushed as eventType `GameStory`, and backfilled into the replay (`GameStoryService.cs:81-93`; `GameHub.cs:134-137`; `GameNotificationService.cs:70-74`). Raw API call: `CallClaudeApi` (`GameStoryService.cs:345-380`).
