...
- Port: `KOTGH_PORT` env var, default 80, binds 0.0.0.0 (`Program.cs:198-200`). The env var is read **only** here; Config holds `Token`, `AnthropicApiKey`, and the default-on `GameStoryEnglishEnabled` test switch (`Config.cs:9-29`, loaded from DataBase/config.json at `Config.cs:16`).
...
- `KOTGH_PORT` is env-only (`Program.cs:198`); Config carries the bot token, Anthropic key, and default-on `GameStoryEnglishEnabled` test switch (`Config.cs:9-29`).
