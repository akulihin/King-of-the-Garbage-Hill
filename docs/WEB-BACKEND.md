...
- **Capture lifecycle**: `CalculateAllFights` clears the prior web fight log before `BeginRound` (`DoomsdayMachine.cs:173-205`); `CaptureRoundResult` freezes only combat/global logs before `RoundNo++` so Тигр bans, Darksci tilt and other next-turn messages cannot enter that stream (`DoomsdayMachine.cs:1517-1522`). After next-round effects and sorting, `FinalizeRound` remaps result players atomically, including only current **awarded bonus** score entries (not pending regular points for a never-played next round), so score/place/death/breakdown agree while action flags and combat stats still come from **PreFightPlayers** (`ReplayService.cs:82-104,301-342`; `DoomsdayMachine.cs:1772-1783`).
