...
> Code-verified against the working tree of 2026-07-12 (v4.3.36). Companion docs: [ARCHITECTURE.md](ARCHITECTURE.md) (§1 process topology, §6 state→screen), [WEB-CLIENT.md](WEB-CLIENT.md) (the Vue SPA consuming this surface), [DISCORD-INTERFACE.md](DISCORD-INTERFACE.md) (the other frontend), and [DAILY-QUESTS.md](DAILY-QUESTS.md) (the quest economy). This doc is the deep catalog of the ASP.NET Core surface: every endpoint, hub method, pushed event, and visibility rule.
...
**Meta (account-level, no game):** `RequestStore()` and `AdjustStoreCharacter(name, percentagePoints)` / `ResetStoreCharacter(name)` / `ResetStoreAllCharacters()` return owner-only `StoreState` snapshots (`GameHub.cs:581-619`). The service exposes only seen rollable characters, validates exact ±1/±10 requests and the 0.50×–2.00× bounds, prices sequential steps from 10 ZBS, calculates exact refunds, and saves every mutation under the account monitor with full rollback (`WebGameService.cs:1243-1441`; DTO `GameStateDto.cs:1061-1082`). Web-created, joined and draft-selected characters enter the same `SeenCharacters` inventory as Discord assignments (`DiscoverStoreCharacter`, `WebGameService.cs:1229-1241`). DooM Guy `RequestDoomFortress()` → `DoomFortressState` and `EquipDoomModule(stage, slotIndex, moduleName)` retain their existing contract. Daily Quest, Loot Box and Achievement calls retain their save-before-publish and acknowledgement behavior: `AcknowledgeLootBox`, `RequestAchievements`, `ClearNewAchievements`, and `AcknowledgeAchievements` remain the public compatibility surface described below and in [DAILY-QUESTS.md](DAILY-QUESTS.md) / [ACHIEVEMENTS.md](ACHIEVEMENTS.md).
...
| Cross-character marks | never serialized as identifying annotations to the affected player's card. Owners see only their own kit's targets/objects through owner widgets or viewer-scoped leaderboard annotations; Goblin mines/Ziggurats, DooM nests, self seal/Шэн state and Napoleon/Support relationships follow the same owner-only rule. Тигр's round-10 `🚫` remains intentionally public as a system-ban joke. Admin inspection remains unmasked; spectators receive no populated custom leaderboard | `GameStateMapper.cs:351-360,360-823`; `GameUpdateMess.cs:219-811`; M35 |
...
|---|---|---|
| Attack | `HandleAttack(player, null, targetPlace)` — `IsAutoMove` is temporarily forced so the handler reads the numeric botChoice instead of Discord component data; on refusal the message is popped from `WebMessages` and returned as the error | `WebGameService.cs:470-500` |
...
| ActivateShen / DeactivateShen / RewriteHistory | direct Salldorum state writes (rewrite steals 1 point per round-loser, +2 psyche, +2 buffered justice, cola time-travel pickup) | `WebGameService.cs:985-1105` |
...
All account-economy operations synchronize on the `DiscordAccountClass` instance: the finish path settles account/history/mastery/ZBS/quest/top-two-box/achievement/statistics together; Battleship, Daily Quest, loot, paid draft picks and both Discord/web stores use that same account monitor (`CheckIfReady.cs:722-796`; `BattleshipService.cs:35-119`; `QuestClass.cs:358-508,638-723`; web store `WebGameService.cs:1243-1391`; Discord store `StoreReactions.cs:196-515`). This prevents simultaneous reward, reroll, pick or shop actions from losing a balance/box/progress update.
...
User-triggered economy writes are durable before confirmation: Daily Quest, loot, achievement acknowledgement, paid draft, and both store transaction paths save while holding the account monitor, then restore their exact in-memory snapshot before returning a retryable error if replacement fails (web store `WebGameService.cs:1252-1391`; Discord store `StoreReactions.cs:196-515`). `SaveAccount` reports the atomic-replace result (`UserAccounts.cs:113-139`; `UsersDataStorage.cs:28-80`). A real-player game-end save failure is critical-logged and retains its settled in-memory state for the periodic retry; bot accounts deliberately skip the immediate disk write (`CheckIfReady.cs:790-796`).
