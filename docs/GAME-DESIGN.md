...
> Code-verified against the working tree of 2026-07-14 (v4.5.6). Every mechanic below was read from the source, not from player-facing descriptions. File references are `path:line` as of this tree.
>
> Companion docs: [ARCHITECTURE.md](ARCHITECTURE.md) (code structure), [CHARACTERS.md](CHARACTERS.md) (all 41 character definitions, including special-purpose entries), [AUDIT-FINDINGS.md](AUDIT-FINDINGS.md) (design-vs-code audit).

## 1. What the game is

A 6-player simultaneous-turn king-of-the-hill game, 10 rounds long. Each player secretly plays one of 41 defined characters (some special-purpose) with hidden stats and passive abilities. Each round every player picks one action; all fights resolve at once; the leaderboard re-sorts by score. After round 10 the player with the most points wins ("Король Мусорной Горы").
...
flowchart TD
    ...
    F --> P{Failed human<br/>Halflife 3 attempt?}
    P -->|yes| Q[Up to 20s:<br/>Freeze or Postpone]
...
On a scheduled **Halflife 3** attempt, Gordon replaces only that round's ordinary settlement: if `P` is his raw pending regular score and `M` is his actual multiplier after Подсчет, the committed amount is `M^P`, not `P × M`. `P ≥ 3` succeeds. On `P < 3`, a human match may pause before score commit for a 20-second Freeze/Postpone choice; postponement 1/2/3 subtracts `1×M`/`2×M`/`3×M` from that failed transformed settlement and retries next round, while the fourth failure cancels (`GordonFreeman.PrepareHalfLifeSettlement`/`ResolveHalfLifeDecision`; `DoomsdayMachine.ResumePendingRound`/`CompleteRoundAsync`).
...
- **Death invariant:** dead players auto-ready only to keep the loop moving; they cannot choose or receive fights, dispatch passives, force targets, trigger predictions or reorder the live game. Ordinary dead seats do not settle pending round score. The two explicit next-round revival passives, Глаз Шусуи and Боги мне не указ, are the only dispatch exceptions. Final ordering always places dead seats below living seats (`CharacterPassives.HandleNextRound`; `DoomsdayMachine.CalculateAllFights`; `Naruto.OrderLeaderboard`).
