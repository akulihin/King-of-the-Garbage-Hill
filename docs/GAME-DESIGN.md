...
> Code-verified against the working tree of 2026-07-01 (v4.1.8). Every mechanic below was read from the source, not from player-facing descriptions. File references are `path:line` as of this tree.
>
> Companion docs: [ARCHITECTURE.md](ARCHITECTURE.md) (code structure), [CHARACTERS.md](CHARACTERS.md) (all 36 characters), [AUDIT-FINDINGS.md](AUDIT-FINDINGS.md) (design-vs-code audit).

## 1. What the game is

A 6-player simultaneous-turn king-of-the-hill game, 10 rounds long. Each player secretly plays one of ~30 characters (36 defined, some special-purpose) with hidden stats and passive abilities. Each round every player picks one action; all fights resolve at once; the leaderboard re-sorts by score. After round 10 the player with the most points wins ("Король Мусорной Горы").
...
Immunities/specials: Boole Family takes no Harm at all; Kimiko guards TheBoys; Испанец converts each Harm into +1 Мораль for mylorik; Спартанец's "Это привилегия…" adds extra Str-pool damage vs top-3 targets from round 5 scaling with skill ratio (`CharacterClass.cs:237-277`).

## 8. Exact round pipeline

Order matters constantly; this is the canonical sequence.

**A. Turn close** (`CheckIfReady.cs:1078-1314`): readiness/timeout → auto-move idlers (pickle-Rick excluded) → Dopa second-action automove → **AWDKA shoved to last place in the list** (`:1113-1127`, see AUDIT-FINDINGS) → bots + auto-movers act → HardKitty shoved last → Геральт skip→block → Aggress forced random attack → Шэн forced attacks from below → Salldorum first-block cola bury → Штормяк taunt (immunities: dead, Котики vs transferred storm, once-per-enemy for the original) → auto-block do-nothings → Монстр no-escape (block/skip stripped; round-10 ban carve-out for Тигр) → round-10 moral dump → reset moral bonus/drop counters → `CalculateAllFights`.

**B. Fight phase** (`DoomsdayMachine.cs:172-1148`): clear web logs, capture replay → flush `BonusPointsFromMoral` → PointFunnel wiring → **DeepCopy GameCharacter→FightCharacter** → conversions: Медитация skip→block, Огурчик Рик block→pickle, Портальная пушка cancels external block/skip, Aggress can't block/skip → Геральт contract fight injection (both directions) → Котики storm pre-pick → **fight loop in leaderboard order** (place 1's attacks resolve first): per fight — `HandleDefenseBeforeFight` → `HandleAttackBeforeFight` → block? (attacker −1 bonus, defender justice) / skip? → Мишень + class skill gains → Steps 1–3 → overrides (IsAbleToWin ±50, Storm ±5, Octopus, Изанаги) → win/loss consequences (§4) → `HandleDefenseAfterFight` → `HandleDefenseAfterBlockOrFight(OrSkip)` → `HandleAttackAfterFight` → `HandleCharacterAfterFight` (both) → `HandleShark` → `ResetFight` (ForOneFight resets on both copies, win/weed streaks, exploit counter).

**C. End of round** (`DoomsdayMachine.cs:1205-1258`): `HandleEndOfRound` (big per-passive dispatcher) → per player: clear flags, auto-ready dead, `SetSpeedResist`, `NormalizeMoral`, **`HandleEndOfRoundJustice`**, **`CombineRoundScoreAndGameScore`** (×1/×2/×4), clear logs, clear PointFunnel → Kratos all-gods-dead check → `RoundNo++`.

**D. Next-round setup** (`DoomsdayMachine.cs:1266-1506`): `HandleNextRound` → save Storm-bite & Ziggurat position locks → **sort by score** → Тигр-топ swap (skipped if round-10-banned; blocked by Ziggurat on #1) → Portal-Gun swap (blocked by Ziggurat) → HardKitty forced last → **LvlUp++ on rounds 3/5/7/9** + place assignment + Мишень roll + place history → restore Ziggurat locks → restore/execute Storm-bite locks & swap → **Quality Drop** → round-10 Goblin-Ziggurat win log → `SortGameLogs` → `HandleNextRoundAfterSorting` → `HandleBotPredict` → `RollExploit` → restart timer.

**E. Game end** (`HandleLastRound`, `CheckIfReady.cs:266-766`): Геральт-6th pitchfork log → **predictions**: +1 bonus per correct guess (+2 for Великий летописец; Кира excluded — uses Death Note; "Выдуманный персонаж" can't be guessed; needs 6 players, ≤5 bots) → M.M. компромат multiplies prediction bonus → TheBoys virus −2/+2 per infected → Цукуеми deductions → sort → AWDKA "Произошел троллинг" (bonus = half of top-1's score at his last winning attack + correct predicts) + re-sort → Premade check (both top-2 & support below carry ⇒ support gets carry−support+1 bonus, re-sort — an *enforced* win) → Sakura "Одна из трех" (top-3 ⇒ narrative win only) → post-game phrases → payouts: ZBS 100/50/40/30/20/10 by place (ties with winner = 100), mastery 10/7/5/3/2/1, loot boxes top-2, pity, achievements, replay save, game removed.

Note the **settlement layers before HandleLastRound**: effects keyed to "round 10 end / round 11" actually live in `HandleEndOfRound` (Пейзаж конца света, Saitama's Ищет достойного противника…), `HandleNextRound` (Чернильная завеса restore…) and `HandleNextRoundAfterSorting` (Запах мусора…) of the round-10 calculation, which runs *before* `HandleLastRound`. Full per-character map in CHARACTERS.md.

## 9. Predictions (Предположения)

Each player assigns a character guess to each enemy (select menus; `GameReactions.cs:553-613`). Editable until round 8 (`ConfirmedPredict` forced open on round 8, auto-confirmed round 9; `CheckIfReady.cs:1333-1343`). Scored at game end (§8E). Bots predict via `HandleBotPredict` heuristics — except at AI difficulty 3, where strict bots (`PlayerType == 404`) from `AiFullKnowledgeRound` (default 3) auto-fill correct predictions for every enemy (`CharacterPassives.cs:6290`). That fill deliberately **skips Монстр без имени**: guessing him can't score (`CheckIfReady.cs:301`) and would gift him +3 / turn the guesser into a pawn on round 9. Кира writes Death-Note names instead of predicting; Монстр без имени can't be guessed and turns players who *are* guessed by him into pawns.
...
- **Bots** (`BotsBehavior.cs:37-64`): round >10 → block; spend moral by place-based thresholds (place 6 hoards to 20, …, place 2 spends at 3; several characters override); level-up; Кира writes notes; target choice via per-game `Nanobot` preference model (justice gaps, positions, Мишень match, previous outcomes). This whole path also drives auto-moved AFK humans (`CheckIfReady.cs:1158`), who therefore inherit the game's difficulty.
- **Bot AI difficulty** (`GameClass.AiDifficulty`, **default 3 everywhere** — changed from 1 on 2026-07-05; the sim's `--ai-difficulty N` flag overrides per-run, range **0-3**): **0** = pure-random experiment baseline (sim-only) — random legal-stat level-up + random attack/block, **skipping** the moral & Kira sub-AIs (moral just auto-cashes at round 10); it still respects the genuine rules a random bot could otherwise violate: characters that literally can't block (`Спарта`/`Aggress`), targets that can't be attacked (`HandleAttack` returns false → retry another), and Макро's two-attack requirement. **1** = the behavior above, unchanged. **2** = smarter within the same skeleton — earlier Мишень captures, uses the class tells it legitimately knows (`KnownPlayerClass`), a per-target justice gradient, a longer stat-loss memory, avoids attacking into predicted Краборак/Толя punish-passives, sharper round-10 block economics and moral-hoard timing. **3** = omniscient from `AiFullKnowledgeRound` (default round 3): bots auto-predict every enemy's true character (except Монстр без имени — see §9) and treat their own predictions and true stats/justice as certain when picking targets. Кира's Death Note is **never** made omniscient at any level (a guaranteed name-kill every round would be degenerate); Eyes pacing is unchanged. Full rule list: `docs/BALANCE-CONSTANTS.md` → "Bot AI difficulty".
...
**Out of scope of this document** (they exist, deliberately not covered here): account meta-systems (quests, achievements, ZBS store, loot boxes, character mastery/pity beyond §11), the replay system, the Discord/web rendering layers, and the unrelated side-games (Battleship, Blackjack) that share the process.
