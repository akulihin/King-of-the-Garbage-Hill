...
Each player assigns a character guess to each enemy (select menus; `GameReactions.cs:553-613`). Editable until round 8 (`ConfirmedPredict` forced open on round 8, auto-confirmed round 9; `CheckIfReady.cs:1333-1343`). Scored at game end (§8E). Bots predict via `HandleBotPredict` heuristics. Кира writes Death-Note names instead; Монстр без имени can't be guessed and turns players who *are* guessed by him into pawns.
