...
>
> Companion docs: [ARCHITECTURE.md](ARCHITECTURE.md) (code structure), [CHARACTERS.md](CHARACTERS.md) (all 38 documented characters), [AUDIT-FINDINGS.md](AUDIT-FINDINGS.md) (design-vs-code audit).
...
**D. Next-round setup** (`DoomsdayMachine.cs:1290-1530`): `HandleNextRound` (Madara: suppress non-combat resources; round 8 global challenge/theme and action lock; round 9 dialogue/seal) → save Storm-bite & Ziggurat position locks → **sort by score** → Тигр-топ swap (skipped if round-10-banned; blocked by Ziggurat on #1) → Portal-Gun swap (blocked by Ziggurat) → HardKitty forced last → **LvlUp++ on rounds 3/5/7/9 except Madara** (DooM Guy maps these to Rune/Shield/Mission/Gun; Let's Roll immediately consumes the point with a random configured module) + place assignment + Мишень roll + place history → restore Ziggurat locks → restore/execute Storm-bite locks & swap → **Quality Drop** → round-10 Goblin-Ziggurat win log → `SortGameLogs` → `HandleNextRoundAfterSorting` (Madara top-1 phrase; entering-round-10 top-1 arms hidden ending) → `HandleBotPredict` → `RollExploit` → restart timer (`CP:4890-4920,6126-6143`; `DoomsdayMachine.cs:1493-1510`).

**E. Game end** (`HandleLastRound`, `CheckIfReady.cs:266-807`): Геральт-6th pitchfork log → **predictions**: +1 bonus per correct guess (+2 for Великий летописец; Кира and Madara excluded; "Выдуманный персонаж" can't be guessed; needs 6 players, ≤5 bots) → M.M. компромат multiplies prediction bonus → TheBoys virus −2/+2 per infected → Цукуеми deductions → sort → AWDKA "Произошел троллинг" (bonus = half of top-1's score at his last winning attack + correct predicts) + re-sort → Premade check (both top-2 & support below carry ⇒ support gets carry−support+1 bonus, re-sort — an *enforced* win) → Sakura "Одна из трех" (top-3 ⇒ narrative win only) → post-game phrases → payouts: ZBS 100/50/40/30/20/10 by **real** place, mastery 10/7/5/3/2/1, loot boxes top-2, pity, achievements. If Madara's hidden ending is active, only final rendering is per-viewer projected (spectators see no result); real score/rewards/history are unchanged, and shared replay/story artifacts are suppressed (`Madara.cs:219-259`; `GameStateMapper.cs:998-1123`; `CheckIfReady.cs:795-807`).
