...
Clamped **0–5** (`CharacterClass.cs:1677-1712`). Gained (next round) by losing a fight, being attacked into your block, or from skills (`AddJusticeForNextRoundFrom{Fight,Skill}`). **Близнец is the block exception**: no generic +1; Monster instead copies the highest blocked attacker's live Justice without draining it (`DoomsdayMachine.cs:591-595`; `CharacterPassives.cs:978-1002`). **Winning any fight this round zeroes your Justice before the buffer lands** (`HandleEndOfRoundJustice`, `CharacterClass.cs:1631-1689`). Effects: weighing term, Step-2 tiebreak, Step-3 window shrink; "Умный" only milks skill from 0-Justice targets. `SeenJusticeNow` (what enemies can infer) only counts fight-sourced justice. Краборак's "Болевой порог" coin-flips each incoming justice point into a regular point instead (`CharacterClass.cs:1654-1673`).
...
**A. Turn close**: readiness/timeout → auto-move idlers (pickle-Rick and round-8/sealed Madara excluded) → Dopa second-action automove → **AWDKA shoved to last place in the list** (see AUDIT-FINDINGS) → bots + auto-movers act → HardKitty shoved last → Геральт skip→block → Aggress forced random attack → Шэн forced attacks from below → Salldorum first-block cola bury → Штормяк taunt → auto-block do-nothings → Монстр no-escape (block/skip stripped; round-10 Тигр carve-out; Naruto siblings excluded from its fallback target pool) → round-8 Madara prediction attacks → sealed-Madara and Naruto-sibling targets are sanitized → round-10 moral dump → `CalculateAllFights` (`CheckIfReady.cs:1389-1441`).

**B. Fight phase**: clear the previous web fight log → capture replay-v2 pre-fight state → flush `BonusPointsFromMoral` → **DeepCopy GameCharacter→FightCharacter** → conversions (Медитация, Pickle, Titan, Portal, Aggress) → Геральт contract injection → sanitize Naruto siblings → **Гарем но джутсу cancels complete finalized attacker queues** → PointFunnel wiring → Madara incoming snapshot → Naruto Justice snapshot → Storm pre-pick → expand Railgun → **fight loop in leaderboard order**. Railgun is known up front, so a side reaching Harem is canceled whole. Conditional BFG branches are appended only after a preceding win; a branch that then reaches Harem cancels the current and every unresolved fight, while an already resolved prefix stands. Each surviving fight follows the ordinary hooks/gates/math/consequences/reset; Расенган writes only `FightCharacter` overrides, while successful Призыв overrides Pickle/Octopus/Izanagi outcomes, creates no Octopus ledger and spends no Izanagi use. After the final round-10 fight, Rumbling resolves kills; **Теневые runs immediately next**, transfers the clone pots and kills/disperses them before `HandleEndOfRound` (`DoomsdayMachine.cs:218-245`; `DoomsdayMachine.cs:381-388`; `DoomsdayMachine.cs:449-545`; `DoomsdayMachine.cs:1508-1510`; `Naruto.cs` `ResolveHaremQueues`, `SettleShadowClones`).
