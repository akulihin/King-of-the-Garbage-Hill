...
**B. Fight phase** (`DoomsdayMachine.cs:172-1236`): clear web logs, capture replay → flush `BonusPointsFromMoral` → PointFunnel wiring → **DeepCopy GameCharacter→FightCharacter** → conversions: Медитация skip→block, Огурчик Рик block→pickle, **Эрен block→Атакующий Титан (no block remains)**, Портальная пушка cancels external block/skip, Aggress can't block/skip → Геральт contract fight injection (both directions) → Котики storm pre-pick → **fight loop in leaderboard order** (place 1's attacks resolve first): per fight — `HandleDefenseBeforeFight` → `HandleAttackBeforeFight` → block? (attacker −1 bonus, defender justice) / skip? → Мишень + class skill gains → Steps 1–3 → overrides (IsAbleToWin ±50, Storm ±5, Octopus, Изанаги) → win/loss consequences (§4) → `HandleDefenseAfterFight` → `HandleDefenseAfterBlockOrFight(OrSkip)` → `HandleAttackAfterFight` → `HandleCharacterAfterFight` (both) → `HandleShark` → `ResetFight` (ForOneFight resets on both copies, win/weed streaks, exploit counter). **After the final fight and before every end-of-round passive, round-10 Rumbling projects post-multiplier score and resolves its kills** (`DM:1235-1236`; `CP:3484-3528`).

**C. End of round** (`DoomsdayMachine.cs:1238-1280`): `HandleEndOfRound` (big per-passive dispatcher, after the Rumbling pre-settlement above) → per player: clear flags, auto-ready dead, `SetSpeedResist`, `NormalizeMoral`, **`HandleEndOfRoundJustice`**, **`CombineRoundScoreAndGameScore`** (×1/×2/×4), clear logs, clear PointFunnel → Kratos all-gods-dead check → `RoundNo++`.
