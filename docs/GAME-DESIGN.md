...
**A. Turn close**: readiness/timeout (plus a live 30-second round-8 Madara bot-reaction gate) → pre-arm an already-active round-10 Вечное Цукуеми total Skip → auto-move idlers → Dopa second-action automove → **AWDKA shoved to last place in the list** → bots + auto-movers act (round-8 strict bots force only the exact Madara prediction) → HardKitty shoved last → Геральт skip→block unless total Цукуеми → Aggress/Шэн/cola/Штормяк/auto-block/Монстр forced-action layers → round-8 Madara prediction attacks → sealed/Naruto targets sanitized → re-snapshot Madara attackers and, if Цукуеми armed on this turn, clear every real action → round-10 moral dump → `CalculateAllFights` (`CheckIfReady.TickAsync`).
