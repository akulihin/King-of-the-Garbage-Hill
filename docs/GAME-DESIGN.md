...
On a scheduled **Halflife 3** attempt, Gordon replaces only that round's ordinary settlement. If `P` is his raw pending regular score, the special multiplier is `P^P` and the committed amount is `P × P^P` (3 raw → 27× → 81 settled). Толя's active Подсчет disables this override and commits the ordinary forced ×1 instead. `P ≥ 3` succeeds. Итачи's active Цукуеми steals the transformed settlement; Осьминожка and Сайтама record only their ordinary round-multiplied point ledgers. On `P < 3`, a human match may pause before score commit for a 20-second Freeze/Postpone choice; postponement 1/2/3 subtracts `1×M`/`2×M`/`3×M` using the attempt's ordinary multiplier and retries next round, while the fourth failure cancels (`GordonFreeman.PrepareHalfLifeSettlement`/`ProjectRegularSettlement`/`ResolveHalfLifeDecision`; `DoomsdayMachine.ResumePendingRound`/`CompleteRoundAsync`).
