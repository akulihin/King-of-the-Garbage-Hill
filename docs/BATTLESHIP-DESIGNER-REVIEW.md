...
The direct 2026-08-09 Captain Flint request adds a third faction and supersedes only the earlier clauses listed below; all unrelated Empire/Alliance rules remain live:

- **Captain Flint setup:** Flint has no budget, regions, upgrades, ordinary four Singles or four summon/Boat slots. The player locks one three-deck Close captain — **Удача** or **Свобода** — and places only it, with every deck outside rows 9–10. The server commits a factual neutral fixed remainder only as a complete set: two 2/2 CloseMelee **Двухпалубный абордажный корабль**, one 2/2 Close **Двухпалубный пушечный корабль** with one ordinary Cannon, one 2/2/2 Close **Трёхпалубный пушечный корабль** with two ordinary Cannons, and the original `fast_warming_ship`. Every otherwise unnamed hull is Speed 1/Space 1. An impossible auto-fill removes every support attempt but leaves the captain placed and the player unready so the captain can be repositioned; a partial remainder is never committed.
- **Warming transfer:** only `fast_warming_ship` moves. It is no longer an Alliance East/25 purchase, loses its region metadata and becomes Flint's fixed 1/1/1/1 Mid hull with Speed 1, Space 1/radius 2 and deck-2 Mast. Its Ballistas are replaced by a separate unlimited damage-2 **Разогрев** action, classified as Cannon; its original first-shot continuation and owner-wide 20-shot overheat/explosion remain. `fast_warming_ship_v2` stays an Alliance East/Home 25-coin definition unchanged.
- **Cannon class:** ordinary **Пушка** deals 3 and supplies two shared shots per live armed deck; initial maximum is 6 with Luck or 12 with Freedom. **Фортуна** deals 4 and has its own 6-shot pool from Luck's three decks. Losing any armed source lowers only that pool's maximum by 2 and clamps its current count. After any Cannon-class shot destroys a deck and leaves exactly one living deck on the same ship/Boarding hull, that last deck is destroyed immediately without a second shot/ammo/reset and the hull sinks normally.
- **Удача:** 2/4/2 HP, with +2 armor on the middle deck, Speed 1 and Fortune on every deck. Its full three-deck Down-facing Boarding entry and each realized movement fire at every lateral enemy deck within 3 of any live Luck deck, without requiring reveal, in stable board order and spending one Fortune per target. **Свобода:** 2/2/2 HP with no armor and ordinary Cannons on every deck; after deck loss its one-successful-use mandatory maneuver offers only exact four-cell axial endpoints. A turn with no legal endpoint skips that barrier without consuming the opportunity. Its full Down-facing Boarding hull moves 4 cells per step. Either captain Devastates a contacted ship of any size, survives and stops that movement.
- **Попугайчик:** one free special boat spawns lazily when that owner's first turn actually begins, on a server-selected free enemy row-1 cell, and immediately scouts Space 1. In dual-Flint, P2's boat does not exist and cannot be targeted before P2's first turn. It moves one cell per cycle and scouts Space 1 after every realized move. Before Flint acts, the owner chooses only a server-legal in-bounds axial direction; if living summons temporarily surround the boat and no option exists, that turn skips the barrier, leaves it stationary and retries later. A selected direction and blur orientation are public, but the opponent receives no destination occupancy/hitbox field: the server immediately moves the logical hittable coordinate while leaving the visual at origin through the opponent turn. The transfer back to Flint animates the move and resolves a shot at that destination or a living arrival; that arrival scouts first, then a living-deck collision removes the boat without damage, otherwise the next direction choice opens when legal.
- **Initiative/privacy:** a sole Flint always receives the first shot; if both players chose Flint, Player 1 receives it. Two Parrots remain independent. The selected direction is public tactical information even though the destination cell is not serialized as occupied; because speed is one, the opponent may infer and fire at it, but the client must not reveal or target-assist the hidden hitbox.
