...
- **Main message** is rebuilt by `UpdateMessage` switching on `MoveListPage` (`GameUpdateMess.cs:1824-1884`): 1 = FightPage + game buttons; 2 = RESERVED (empty; legacy logs page); 3 = LvlUpPage + level-up select (+ the decorative disabled `crutch` button "Riot style choice" for round-9 Дизмораль, `GameUpdateMess.cs:1859-1864`); 4 = debug (commented out); 5 = AramPickPage + reroll buttons; 6 = DraftPickPage + pick buttons (`GameUpdateMess.cs:1699-1761`).
- **FightPage anatomy** (`GameUpdateMess.cs:1052-1180`): title "King of the Garbage Hill", footer = `GetTimeLeft` (`(N/300с) | Версия…`, plus `Ожидаем других игроков •` when ready; mylorik/DeepList get the `(x+х)*19` gag — `GameUpdateMess.cs:1891-1903`); description = global logs (admin-hidden snippets stripped for non-admins, `GameUpdateMess.cs:1079-1084`) + stat block (Интеллект/Сила/Скорость/Психика + Справедливость/Мораль/Скилл + Мишень + Класс + `Множитель очков`, `GameUpdateMess.cs:1108-1124`) + the leaderboard (`LeaderBoard`, `GameUpdateMess.cs:158-191` — own score only; per-viewer prefixes/suffixes via the same CustomLeaderBoard methods the web reuses); fields `События прошлого раунда:` and `События этого раунда:` from personal logs split by `|||`, chunked at 1024 chars (`GameUpdateMess.cs:1127-1171`); avatar thumbnail unless `IsMobile` (`GameUpdateMess.cs:1174-1175`).

## 5. Buttons & selects catalog (main game message)

Composed by `GetGameButtons` (`GameUpdateMess.cs:1600-1670`): row 0 — Блок, Авто Ход (Tier > 3, non-ARAM, `GameUpdateMess.cs:1606-1609`), Изменить свой выбор, Завершить Игру, Дебаг (two owner IDs only, `GameUpdateMess.cs:1613-1616`); row 1 — attack select; row 2 — moral/skill buttons (`GameUpdateMess.cs:1620-1624`); row 3 — predict select (non-ARAM, hidden for the AdminPlayerType passive, `GameUpdateMess.cs:1626-1632`); row 4 — character buttons + Mobile Device in round 1 (`GameUpdateMess.cs:1635-1667`).

| custom-id | Label (exact) | Built at | Handled at |
|---|---|---|---|
| `block` | `Блок` (Success; disabled when acted or round > 10 unless Kratos-revenant) | `GameUpdateMess.cs:1763-1778` | `GameReactions.cs:315-365` (Спарта → "Спартанцы не капитулируют!!" `GameReactions.cs:316-320`; Aggress → "I. WONT. STOP." `GameReactions.cs:322-326`; Dopa `Макро` two-action logic `GameReactions.cs:329-352`) |
...
| `auto-move` | `Авто Ход` (locked first 29 s except owners) | `GameUpdateMess.cs:1814-1822` | `GameReactions.cs:166-184` |
| `change-mind` | `Изменить свой выбор` (Dopa gets disabled `선택 변경`) | `GameUpdateMess.cs:1803-1812` | `GameReactions.cs:186-212` (un-readies, strikes the log line through) |
| `end` | `Завершить Игру` (Danger) | `GameUpdateMess.cs:1780-1795` | `GameReactions.cs:297-300` → `EndGame` (§8) |
| `confirm-prefict` [sic] | `Я подтверждаю свои предположения` | `GameUpdateMess.cs:1552-1554` | `GameReactions.cs:290-296` |
| `confirm-skip` | `Я подтверждаю пропуск хода` | `GameUpdateMess.cs:1555-1557` | `GameReactions.cs:214-224` |
| `moral` | `на N бонусных очков` ladder 20/13/8/5 морали → 10/5/2/1; DeepList disabled "Интересует только скилл"; М.М. компромат disables | `GameUpdateMess.cs:1520-1548` | `GameReactions.cs:366-368` → `HandleMoralForScore` |
| `skill` | `Обменять N Морали на M Cкилла` ladder 20→100 … 1→2 (Еврей extra 7→40 `GameUpdateMess.cs:1577-1579`); Булькает disabled | `GameUpdateMess.cs:1550-1598` | `GameReactions.cs:369-371` → `HandleMoralForSkill` |
| `predict-1` | placeholder `Сделать предположение`, options `{name} это...`; disabled from round 9; Булькает → `Бууууууль` | `GameUpdateMess.cs:1440-1476` | `GameReactions.cs:385-387` → `HandlePredic1` (builds `predict-2` with all character names, `GameReactions.cs:556-583`) |
...
| `lvl-up` | placeholder `Выбор прокачки` (Вампур_ garlic gag `GameUpdateMess.cs:1482-1483`; Ирелия `Выбор нерфа` `GameUpdateMess.cs:1485-1488`; round-9 Дизмораль psyche-only menu `GameUpdateMess.cs:1502-1512`), options Интеллект/Сила/Скорость/Психика = values 1-4 | `GameUpdateMess.cs:1479-1498` | `GameReactions.cs:373-376` → `HandleLvlUp` (`GameReactions.cs:619-628`) |
| `mobile-device` | `Mobile Device` (round 1 only) | `GameUpdateMess.cs:1386-1389` `GameUpdateMess.cs:1664-1667` | `GameReactions.cs:159-164` (drops the thumbnail) |
| `debug_info` | `Дебаг` | `GameUpdateMess.cs:1797-1800` | `GameReactions.cs:308-312` (toggles the dead page 4) |
| `stable-Darksci` / `not-stable-Darksci` | `Мне никогда не везёт...` / `Мне сегодня повезёт!` (round 1, + hint DM) | `GameUpdateMess.cs:1638-1649` | `GameReactions.cs:226-254` |
| `yong-gleb` | `Вспомнить Молодость` (round 1) | `GameUpdateMess.cs:1652-1656` | `GameReactions.cs:256-288` (in-place transform to Молодой Глеб) |
| `dopa-attack-select` | placeholder `Второе Действие` (Korean gags when skipped/gg) | `GameUpdateMess.cs:1391-1438` | **no case in the dispatch switch — selections are silently ignored** (finding **m23** in AUDIT-FINDINGS.md; Dopa's real second action runs through the normal attack/block `Макро` branches, `GameReactions.cs:329-352` `GameReactions.cs:730-737`) |
| `aram_reroll_1..4` / `aram_reroll_5` / `aram_roll_confirm` | `Reroll #N` (max 4 passive rerolls) / `Reroll Stats` (max 1) / `Confirm` → disabled `Wait for other players` | `GameUpdateMess.cs:1672-1696` | `GameReactions.cs:393-416` |
| `draft_pick_0` / `draft_pick_1` / `draft_pick_2` | `{Name} (FREE)` / `{Name} (cost 5 ZBS points)`; after pick `Ожидаем остальных` (`draft_pick_wait`, no handler) | `GameUpdateMess.cs:1736-1761` | `GameReactions.cs:417-421` → HandleDraftPick |
...
**Attack trace** (`HandleAttack`, `GameReactions.cs:630-746`): parses the selected option as the target's player Guid (`GameReactions.cs:649-650`) — or, for bots/web (`IsAutoMove` trick, WEB-BACKEND.md §8), by leaderboard place (`GameReactions.cs:654`) — then **records `WhoToAttackThisTurn`** (`GameReactions.cs:660`), the state the fight loop consumes. Guards: pickle-Rick auto-confirm (`GameReactions.cs:664-665`), Клинки хаоса splash adds neighbours (`GameReactions.cs:669-679`), Weedwick/DeepList pet refusals (`GameReactions.cs:689-701`), round-10 Butcher ban (`GameReactions.cs:705-710`), СОсиновый кол repeat-target ban (`GameReactions.cs:713-718`), self-attack `Зачем ты себя бьешь?` (`GameReactions.cs:721-727`), Макро first-of-two (`GameReactions.cs:730-737`); otherwise ready + log `Вы напали на игрока {name}` (`GameReactions.cs:740-745`).
...
- `dopa-attack-select` renders but has no dispatch case (finding **m23**, `GameUpdateMess.cs:1420-1425`); `confirm-prefict` is a load-bearing typo (`GameUpdateMess.cs:1552-1554`); the `stats` case toggles the empty page 2 but no button builds it anymore (`GameReactions.cs:302-306`).
- The `Дебаг` button (`GameUpdateMess.cs:1797-1800`) and early Авто Ход unlock are hardcoded to two owner Discord IDs (`GameUpdateMess.cs:1613` `GameUpdateMess.cs:1818-1819`).
