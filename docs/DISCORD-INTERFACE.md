...
- **FightPage anatomy** (`GameUpdateMess.cs:1068-1205`): title "King of the Garbage Hill", footer = `GetTimeLeft` (`(N/300с) | Версия…`, plus `Ожидаем других игроков •` when ready; mylorik/DeepList get the `(x+х)*19` gag); description = global logs (admin-hidden snippets stripped for non-admins) + stat block (Интеллект/Сила/Скорость/Психика + Справедливость/Мораль/Скилл + Мишень + Класс + `Множитель очков`) + the leaderboard. Madara's variant keeps base stats, Justice and Class but removes all Harm-resist, Moral, Skill and target rows (`GameUpdateMess.cs:1141-1178`). The two personal-log fields and avatar behavior are otherwise unchanged.
