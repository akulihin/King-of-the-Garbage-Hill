...
- Gordon's `PassiveAbilityStatesDto.Gordon` is likewise owner-only: it carries crowbar/fight progress, current HEV Justice boost, wake state, active headcrab targets and countdowns, zombie/rescue totals, and the Halflife 3 release/decision payload. The corresponding 🦀/🧟 leaderboard markers are rendered only in Gordon's viewer projection; affected players and spectators receive no identifying mark (`GameStateMapper.cs:427-485`; `GameUpdateMess.cs` `CustomLeaderBoardBeforeNumber`).
...
- Active `Вечное Цукуеми` is an **authoritative round-10 total Skip plus final per-viewer projection**: all real queues are cleared before combat, fight conversions/injections are suppressed, and only the ordinary non-combat/post-round pipeline mutates scores. Madara and a Gordon who spent `Просыпайтесь, мистер Фримен` when the active Цукуеми exposed its round-9 premonition receive that real empty round and real result; every other non-Madara viewer receives output-only synthetic wins/place/score-source, with their fight animation completing before the podium/theme; spectators receive no result data (`Madara.PrepareEternalTsukuyomiRound`; `GordonFreeman.SeesEternalTsukuyomiReality`; `GameStateMapper.ApplyEternalTsukuyomiProjection`; `Game.vue`; `FightAnimation.vue`). A replay/story is shared rather than viewer-scoped, so activated games still suppress both artifacts (`CheckIfReady.HandleLastRound`; `GameNotificationService`).
