...
- **Parallel sims are safe, but only because the harness owns no disk state** (finding M52): simulation mode sets `UserAccounts.DisableDiskPersistence` before the container is built (`Program.cs:40`), so the process starts with an empty in-memory account store and never writes one. It only plays bot accounts and resets them at start anyway. This is what makes one sweep per difficulty level, four at once, work: sharing the account folder previously crashed a starting process (a listed file vanished mid-flush → SIGABRT/exit 134) *and* invalidated the experiment, because each start resets every bot account's pity/history while the other arms are mid-run. Anything new the harness persists must be per-process or it reintroduces both bugs.
