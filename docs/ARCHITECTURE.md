...
| 15 | `HandleBotPredict` (7371) | `DoomsdayMachine.cs:2023` | difficulty-routed bot prediction heuristics; L2/L3 use the fair viewer projection |
...
Score mutation has one central floor contract: `AddBonusPointsCore` and `AddScoreWithMultiplier` clamp ordinary characters at zero; HardKitty bypasses by passive and only audited lethal effects call `AddBonusPointsIgnoringFloor` (Kira arrest, Geralt pitchfork). Dead seats do not flush buffered regular points. `GameClass.WinnerPlayerIds` records the authoritative declared winner(s) independently of raw place so profile settlement, special wins and simulation reports share one result source (`InGameStatusClass`; `CheckIfReady.HandleLastRound`).
...
- **Entry**: `Program.cs:61` — after the Lamar container is built (the `CheckIfReady` 100ms timer is already running from its constructor), `--sim` branches into `SimulationRunner.RunAsync` (`Program.cs:61-70`) and exits with its code; Discord login/Kestrel are never reached. `ClaudeHaikuService.Disabled` is set first (`Program.cs:63`) so sims never spend Anthropic API credits (Geralt hints fall back to static), and `ReplayService.CaptureEnabled` is set false (`Program.cs:67`): the harness never saves or serves a replay, while capturing one costs a full six-player state projection — localization included — three times per round, which used to dominate the simulator's runtime and heap (finding M51; WEB-BACKEND.md §9).
...
- **Forced line-ups**: `forcedCharacters` on `HandleCharacterRoll` (`StartGameLogic.cs:101-102`) assigns characters by list order, bypassing the roll (`StartGameLogic.cs:194-223`). Needed because `CharacterToGiveNextTime` only works for non-null IUser slots (part #1 reservation, `StartGameLogic.cs:138-161`). The caller owns LeCrisp/Толя and ≤1 Tier-4 constraints. Natural coverage still excludes `TeamModeOnly`; explicit `--characters` validation intentionally permits team-only characters through the existing admin/test force path, so Napoleon and Support pilots can be measured (`SimulationRunner.cs:121-154`; `BotGameFactory.cs:42-50`).
...
- **Parallel sims are safe, but only because the harness owns no disk state** (finding M52): `--sim` sets `UserAccounts.DisableDiskPersistence` before the container is built (`Program.cs:39`), so the process starts with an empty in-memory account store and never writes one. It only plays bot accounts and resets them at start anyway. This is what makes "one sweep per `--ai-difficulty` level, four at once" work: sharing `DataBase/UserAccounts` previously crashed a starting process (a listed file vanished mid-flush → SIGABRT/exit 134) *and* invalidated the experiment, because each start resets every bot account's pity/history while the other arms are mid-run. Anything new the harness persists must be per-process or it reintroduces both bugs.
- **Why per-game cost is a sweep-correctness issue, not just a speed issue** (finding M51): one tick pass advances *every* live game on *one* thread, so the wall time between two rounds of the same game scales with the batch size. When per-game cost regressed ~10x in July 2026, a 2 000-game pass exceeded the 30s stuck threshold (`SimulationRunner.cs:28`) and the watchdog culled ~1 600 perfectly healthy games as "STUCK at round 2" — and removing a game mid-tick then crashed the bot that was inside it. A sweep reporting mass STUCK at an early round with `secondsStalled == 30` and `gamesFinished == 0` is the signature of *the simulator being slow*, not of frozen games: profile before hunting a game-logic bug. The simulator therefore keeps the round pipeline free of viewer-facing work — `ClaudeHaikuService.Disabled` and `ReplayService.CaptureEnabled = false` (`Program.cs:61-70`), the latter because capturing a replay nobody reads costs a six-player state projection three times per round (WEB-BACKEND.md §9).
