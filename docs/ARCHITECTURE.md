...
- **AI difficulty** `--ai-difficulty N` (1-3, default 1; validated at `SimulationRunner.cs:88` → exit 2 if out of range; echoed in the console mode line and in `options.aiDifficulty` of the report; threaded through `CreateBotGameAsync(..., aiDifficulty:)` `BotGameFactory.cs:47,82` onto the game's `AiDifficulty` (`GameClass.cs:69`)). **1** = legacy bots (bit-for-bit unchanged — the only value any non-sim path ever sets); **2** = smarter heuristics on the same decision skeleton; **3** = bots know every enemy's character from `AiFullKnowledgeRound` (`GameClass.cs:72`, default 3) via auto-filled predictions, plus certainty-based targeting. Sim-only: no Discord/web UI sets it. Rule catalogue in `docs/BALANCE-CONSTANTS.md` → "Bot AI difficulty".
