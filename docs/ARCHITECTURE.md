...
- Account economy mutations that can race (game-end rewards and lobby loot opens) must hold the same account monitor. Real-player reward/open/ack changes are saved immediately; persistence serializes under that monitor and replaces the account JSON through a same-directory temporary file rather than exposing a partially written document (`CheckIfReady.cs:705-777`; `QuestClass.cs:318-368`; `UserAccounts.cs:113-139`; `UsersDataStorage.cs:28-76`). The loader accepts only canonical numeric account filenames (`UsersDataStorage.cs:81-110`).
