...
| `Game/GameLogic/BotsBehavior.cs` | 2854 | bot AI + Nanobot preference model; difficulty-gated heuristics (`Smart()`/`Omni()`, L1 default & unchanged) |
...
- **Modes** (flag parsing `Game/Simulation/SimulationRunner.cs:72-77`): smoke `--games N` = natural bot roll (tier-skewed — bots skip Tier<4 except Кира, so smoke alone does NOT cover all characters); coverage `--coverage K` = generated line-ups so every rollable non-team character appears ≥ K times (`SimulationRunner.cs:116`); matchup `--characters "6 comma-separated names"` = fixed line-up, validated (`SimulationRunner.cs:98`), for bug repros and testing a just-changed character. Smoke+coverage are additive; matchup replaces both.
- **AI difficulty** `--ai-difficulty N` (0-3, **default 3**; validated at `SimulationRunner.cs:86` → exit 2 if out of range; clamped to 0–3 at `BotGameFactory.cs:82`; echoed in the console mode line and in `options.aiDifficulty` of the report; threaded through `CreateBotGameAsync(..., aiDifficulty:)` `BotGameFactory.cs:47,82` onto the game's `AiDifficulty` (`GameClass.cs:69`)). **0** = pure-random baseline for experiments — random legal-stat level-up + random attack/block, **skipping** the moral & Kira sub-AIs and respecting the real cannot-block (`Спарта`/`Aggress`) / cannot-attack (`HandleAttack` returns false) / Макро two-attack rules (`Dumb()` `BotsBehavior.cs:41`, `HandleBotAttackRandom` `:2676`); **1** = legacy bots (bit-for-bit unchanged); **2** = smarter heuristics on the same decision skeleton; **3** = bots know every enemy's character from `AiFullKnowledgeRound` (`GameClass.cs:72`, default 3) via auto-filled predictions, plus certainty-based targeting. No Discord/web UI exposes a picker — real games take the `AiDifficulty` **default (now 3, was 1)**; only the sim overrides it per-run via the flag (0 is sim-only). Rule catalogue in `docs/BALANCE-CONSTANTS.md` → "Bot AI difficulty".
- **AI measurement probe** `--ai-probe N` (0-3; validated `SimulationRunner.cs:94`) + optional `--ai-probe-char "Name"` — runs **one** bot at difficulty `N` while the rest of the field stays on `--ai-difficulty` (per-player override `GamePlayerBridgeClass.AiDifficulty`, default -1 = inherit; resolved by `EffectiveDifficulty` `BotsBehavior.cs:39` and the L3 fill `CharacterPassives.cs:6291`). With `--ai-probe-char` the override targets whichever player rolled that character (so it aggregates across coverage games) — else slot 0. This is the A/B instrument for "does a higher level pilot a character better?", which homogeneous runs can't answer (a universal buff like prediction points cancels out in relative winrates). Echoed in the console line and `options.aiProbe`/`aiProbeChar`. Threaded via `CreateBotGameAsync(..., aiProbe:, aiProbeChar:)` `BotGameFactory.cs:47,88`.
...
- **Seeded determinism + paired A/B** `--seed N`: runs games **sequentially, one at a time**, reseeding the game RNG per game (`SecureRandom.SetSeed`, `Helpers/SecureRandom.cs`) and reproducing the line-up plan — so a fixed seed replays the batch and two runs differ only by what you change. The background round timer is disabled and games are pumped via `CheckIfReady.TickAsync` (single-threaded → deterministic). `--ab-char "Name" [--ab-test N=3] [--ab-control N=1]` does an **in-process paired A/B**: one process plays the seeded line-ups twice (control arm, then test arm, same per-game seeds) and prints the paired per-game win%/avgPlace/avgScore delta with a 95% CI + verdict (wrapper: `bash tools/ab.sh <char> [test] [coverage] [seed] [control]`). This is the low-noise way to answer "does a higher AI level pilot this character better than L1?". To make seeding bite, the previously-unseeded `new Random()` (initial Мишень roll `CharacterClass.cs`, Краборак Болевой-порог) and `Guid.NewGuid()` shuffles/identities (`PlayerId`, seating, TheBoys marks) were routed through `SecureRandom` (crypto-random when unseeded, so real games are unchanged). **Caveat:** a few hash-order-sensitive characters (e.g. Стая Гоблинов, Котики) keep residual per-run nondeterminism the seed can't fully pin — raise coverage for those; the A/B prints a self-test line when test==control.
