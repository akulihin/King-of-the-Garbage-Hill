...
- Эрен splits owner state (`PassivesClass.Eren`) from hatred marks stored on each affected player's `ErenHatredMark`; the owner-only DTO aggregates those per-player marks (`PassivesClass.cs:267-274`; `GameStateMapper.cs:372-396`). His round-wide Titan boost is implemented as a per-fight `FightCharacter` override reapplied in both attack/defense hooks, because `ResetFight` correctly clears each override after one fight (`CharacterPassives.cs:62-72,443-447,1002-1006`).
- Мадара keeps match state in `PassivesClass.Madara`, not `CharacterClass`, so its `HashSet<Guid>` is persistent per player and does not belong in `CharacterClass.DeepCopy` (`PassivesClass.cs:169-173`; `Madara.cs:18-32`). His combat Skill 100 is still written to `FightCharacter` in both before-fight hooks (`CharacterPassives.cs:461-467,1026-1032`). On round 8, the post-round human prediction reset preserves Madara's ready/confirmed unable-to-act state and the readiness precommit reasserts it every tick; live non-skipping strict bots spend pending level-ups and are then pre-committed to ordinary Madara attacks before human readiness is counted (`CheckIfReady.cs:1038-1042,1463-1487`; `BotsBehavior.cs:72-132`; `Madara.cs:197-230`). The hidden `Вечное Цукуеми` ending is deliberately a **final per-viewer projection** after normal `MapPlayer`: non-Madara viewers receive a synthetic winning fight/place/score-source and only their own applicable victory theme, Madara receives the authoritative result, and spectators receive no result data; the underlying `GameClass` and payouts remain real (`GameStateMapper.cs:922-1047`; `Game.vue:233-274`; `Madara.cs:242-282`). A replay/story is shared rather than viewer-scoped, so activated games suppress both artifacts instead of persisting a result that reveals the illusion (`CheckIfReady.cs:806-818`; `GameNotificationService.cs:78-88`).
