...
- **AI difficulty** `--ai-difficulty N` (1-3, **default 3**; validated at `SimulationRunner.cs:88` → exit 2 if out of range; echoed in the console mode line and in `options.aiDifficulty` of the report; threaded through `CreateBotGameAsync(..., aiDifficulty:)` `BotGameFactory.cs:47,82` onto the game's `AiDifficulty` (`GameClass.cs:69`)). **1** = legacy bots (bit-for-bit unchanged); **2** = smarter heuristics on the same decision skeleton; **3** = bots know every enemy's character from `AiFullKnowledgeRound` (`GameClass.cs:72`, default 3) via auto-filled predictions, plus certainty-based targeting. No Discord/web UI exposes a picker — real games take the `AiDifficulty` **default (now 3, was 1)**; only the sim overrides it per-run via the flag. Rule catalogue in `docs/BALANCE-CONSTANTS.md` → "Bot AI difficulty".
