...
- The boolean flags declared once per `HandleBotAttack` invocation (`isTargetTooGood`, `isLostLastRoundAndTargetIsBetter`, `isJusticeTheSame`, `isTargetFirst`, … `BotsBehavior.cs:594-599`) latch to `true` when set for any target and are **never cleared between iterations** of the `foreach (var target in allTargets)` loop. A handful of per-character compensations read these flags *after* the loop or on a later target and so can fire for a target that never actually received the penalty — e.g. mylorik's compensation (`BotsBehavior.cs:1074`) and Глеб's (`:1235`) key off `isLostLastRoundAndTargetIsBetter`.
