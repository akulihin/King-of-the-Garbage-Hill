...
| Стая Гоблинов | Location, Location, Ziggurat / Место, место и Зиккурат<br>`c_goblin_architect` · Common / 10 ZBS | Build the first Ziggurat. | Built Different / Особая постройка<br>`c_goblin_summit` · Legendary / 228 ZBS + 2 boxes | Actually receive the Ziggurat's enforced win: with a Ziggurat at place 1, be **below** 1st after the final score sort so `HandleLastRound`'s bonus-point overtake fires, and finish alive at actual place 1. Winning on score with a place-1 Ziggurat does not qualify, however early it was built (m57). |
...
|---|---:|---:|---|
| Common | 60% | 15–30 | +1 percentage point of permanent roll weight |
| Uncommon | 25% | 40–75 | +2 percentage points of permanent roll weight |
| Rare | 12% | 100–175 | +3 percentage points of permanent roll weight |
| Epic | 2.5% | 300–450 | +5 percentage points and that Tier 1–2 character queued for the next newly created game |
| Legendary | 0.5% | 750 | +10 percentage points and that Tier 1 character queued for the next newly created game |

The server owns the table and every roll; the client receives the table only for explanation. Rarity uses the shared cryptographic RNG over 10,000 equally likely outcomes with exact cumulative thresholds, and variable rewards use its inclusive bounds (`QuestService.LootBoxOdds`, `QuestClass.cs:285-293`; `RollLootBoxTier`, `QuestClass.cs:979-988`). A box is earned by finishing alive in the reward top 2, including Sakura's first-place reward treatment; Epic/Legendary achievements and Daily Quest 3/3 mastery add boxes to the same inventory (`CheckIfReady.cs:681-684,736-743`; quest mastery `QuestClass.cs:663-668`). Existing pending inventory is preserved and uses the current table when opened.

Every box also selects one public Tier ≥ 0 character. Epic narrows that selection to Tier 1–2 and Legendary to Tier 1; their selected character is both boosted and added to the account's FIFO next-game queue. Lower rarities may select any public character. The character becomes visible in that account's Store immediately. Its bonus is added to the Store's paid adjustment to form the effective roll multiplier, is permanent/non-refundable, and stops at the shared ×2.00 effective cap; selection prefers a character still below that cap (`QuestService.GenerateLootBox`; `DiscordAccountClass.CharacterChances.GetEffectiveMultiplier`). unknown_bug and every negative-tier definition are excluded.
...
- A new open decrements exactly one pending box, rolls and credits its ZBS plus character reward, and stores a result snapshot containing rarity, amount, character/boost/queue data, balance, remaining inventory, pity state, timestamp and `WasPityUpgrade` (`QuestService.OpenLootBox`/`GenerateLootBox`).
