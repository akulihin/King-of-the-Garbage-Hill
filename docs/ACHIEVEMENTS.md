...
| Стая Гоблинов | Location, Location, Ziggurat / Место, место и Зиккурат<br>`c_goblin_architect` · Common / 10 ZBS | Build the first Ziggurat. | Built Different / Особая постройка<br>`c_goblin_summit` · Legendary / 228 ZBS + 2 boxes | Actually receive the Ziggurat's enforced win: newly build or enter the place-1 Ziggurat during round 10, have its score-free first-place placement change the natural result, and finish alive at actual place 1. An old place-1 construction, continuous earlier occupancy, or a natural score win does not qualify (M205–M206; m57). |
...
| Salldorum | Open Happiness / Открой счастье<br>`c_salldorum_cola` · Uncommon / 25 ZBS | Activate the Time Capsule once; the free matching-history activation counts as a drink. | History Repeats Itself / История повторяется<br>`c_salldorum_double_cola` · Legendary / 228 ZBS + 2 boxes · **secret, unbound** | The free history activation does not consume a buried cola, so either activation order can reach two drinks. The only description/hint copy remains «Колу можно выпить дважды, если знать историю.» `CharacterNames` is empty. |
...
| Наруто (original) | Believe in the Harem / Поверь в гарем<br>`c_naruto_harem` · Rare / 50 ZBS | Receive 3 successful donations with Гарем но джутсу. One attacker donates at most once per activation; later activations can advance progress again, while clone actions and protected/rejected transfers do not count. | Shadow Hokage Dividend / Дивиденды теневого Хокаге<br>`c_naruto_rasengan` · Legendary / 228 ZBS + 2 boxes | Receive at least 30 points from Теневые and finish alive at actual place 1. |
...
|---|---|---|---|
| `x_spartan_dragon` | Dragon Slayer / Убийца драконов | Загадочный Спартанец в маске triggers DragonSlayer against round-10 Sirinoks/Дракон below her 228-Skill autowin-protection threshold and actually wins that fight. Spartan earns it. | Epic · 100 ZBS + 1 box |
...
There is no retroactive reconstruction for later expansion cards because most requirements need per-fight/per-passive facts that history never stored. Their bests and unlocks therefore begin with matches completed after deployment; the original 34 cards continue from their existing V2 state.

## 8. Loot boxes V2

### Base odds and rewards

| Rarity | Base chance | ZBS reward (inclusive) | Character reward |
|---|---:|---:|---|
| Common | 60% | 15–30 | — |
| Uncommon | 25% | 40–75 | — |
| Rare | 12% | 100–175 | — |
| Epic | 2.5% | 300–450 | one Tier 1–2 character queued for the next newly created game |
| Legendary | 0.5% | 750 | one Tier 1 character queued for the next newly created game |

The server owns the table and every roll; the client receives the table only for explanation. Rarity uses the shared cryptographic RNG over 10,000 equally likely outcomes with exact cumulative thresholds, and variable rewards use its inclusive bounds (`QuestService.LootBoxOdds`; `QuestService.RollLootBoxTier`; `SecureRandom`). A box is earned by finishing alive in the authoritative final top 2; qualifying Sakura/Goblin guarantees first move their holder to actual place 1 without changing score. Epic/Legendary achievements and Daily Quest 3/3 mastery add boxes to the same inventory (`CheckIfReady.HandleLastRound`; quest mastery in `QuestService`; M206). Existing pending inventory is preserved and uses the current table when opened.

Only Epic and Legendary select a character. Epic selects a public Tier 1–2 character and Legendary selects Tier 1, adds it to the account's FIFO next-game queue and reveals its Store card. Common, Uncommon and Rare contain only ZBS. No loot-box result creates or mutates `CharacterChance`; manual Store purchases are the sole character roll-weight modifier (`QuestService.GenerateLootBox`; `DiscordAccountClass.CharacterChances.GetEffectiveMultiplier`). Tier 0/PRO, unknown_bug and every negative-tier definition are excluded from guaranteed-character loot pools.

Queued characters are consumed only by a newly created normal game, not by joining an already-running game. Several Epic/Legendary rewards accumulate instead of overwriting one another. Discord creation reserves the next queue entry before natural rolls; web creation replaces or claims the matching bot seat. The rewarded assignment is final rather than a paid draft alternative. If two players queue the same unique roster character, the first reservation receives it and the other reward remains queued (`StartGameLogic.PeekNextCharacterAssignment`/`ConsumeNextCharacterAssignment`; `WebGameService.CreateGame`; `General.StartGame`). This changes acquisition only; character mechanics and match rules are untouched.

### Rare+ pity

The loot-box pity counter records consecutive final Common/Uncommon results. After 9 such boxes, the 10th opening is guaranteed Rare+: a natural Rare/Epic/Legendary is preserved, while a natural Common/Uncommon is upgraded to Rare. Any Rare+ result resets the counter to 0 (`QuestService.NormalizeLootBoxPity`/`GenerateLootBox`). `GuaranteedRareIn` is server-derived and both the lobby and reveal screen visualize the remaining distance.
...
- Opening is serialized on the account monitor. If a prior result has not been acknowledged, repeated `OpenLootBoxV2` calls return that same `OpeningId` and do not consume another box or grant ZBS again (`QuestService.OpenLootBox`; hub transaction `GameHub.OpenLootBoxCore`).
- A new open decrements exactly one pending box, rolls and credits its ZBS plus any Epic/Legendary guaranteed character, and stores a result snapshot containing rarity, amount, optional character/queue data, balance, remaining inventory, pity state, timestamp and `WasPityUpgrade`. Character roll weights are outside this transaction (`QuestService.OpenLootBox`/`GenerateLootBox`).
- `AcknowledgeLootBox(openingId)` marks only the matching current result; stale/mismatched IDs are safe no-ops (`QuestService.AcknowledgeLootBox`).
- `RequestQuests` snapshots under the account monitor and includes `LastUnacknowledgedLootBox`, odds and pity fields alongside Daily Quest V2. After a disconnect/reload, the client resumes that already-determined reveal, with current account balance/inventory mapped over the historical result snapshot, rather than rolling again (`GameHub.BuildQuestState`; `QuestStateDto`).
- V2 opens and loot/achievement acknowledgements save before confirming; write failure restores the exact affected account snapshot and returns a retryable hub error. Cached pre-V2 clients still call legacy `OpenLootBox`, which acknowledges inside the same durable transaction so they can advance without the new acknowledgement call (`GameHub.OpenLootBoxCore`/`AcknowledgeLootBoxCore`). Paid draft/shop operations share the account monitor too. `SaveAccount` reports atomic-replace success; game-end save failure is critical-logged with its settled in-memory state retained for the periodic retry (`UserAccounts.cs` `SaveAccount`; `UsersDataStorage.cs:28-80`; `CheckIfReady.cs:812-818`).
...
The loot-box dialog is staged: anticipation/opening first, then the already server-determined result; rarity-specific treatment, ZBS, the optional Epic/Legendary character portrait/tier and next-game queue receipt, current balance/inventory/pity, complete odds disclosure and the pity-upgrade badge share one modal (`LootBox.vue`). No chance boost is displayed. The lobby reward card keeps the FIFO head and remaining queue count visible after acknowledgement; Store cards show only manually purchased adjustments (`Lobby.vue`; `Store.vue`). The store owns reward-modal priority: Lobby asserts it, waits one render tick for any achievement focus trap to unmount, then shows loot; handoff back also waits a tick (`Lobby.vue` loot flow). A bounded server/reveal wait becomes bilingual Retry reveal / Return to lobby UI, and a late result safely reopens; acknowledgement errors likewise remain inline and retryable (`game.ts` loot wrappers; `LootBox.vue`). Keyboard isolation/restoration, Escape behavior, ARIA semantics, compact mobile layout and reduced-motion fallbacks remain part of the contract. Daily Quest completion stays inline and therefore never adds another focus trap; reroll explicitly announces and focuses its replacement (`DailyQuestBoard.vue:227-303,515-604`).
...
`audit-achievements.sh` verifies the exact IDs/category counts, duplicate absence, evaluator references, required bilingual/reward metadata, and all 41 declared Normal/Hard character pairs against the eligible `characters.json` roster names. The build checks the SignalR DTO/TypeScript mirrors; the simulation protects the gameplay hooks used as observations.
