...
Jon's fixed defeat, Server-King and Castle entry/final phrases use five paired `PhraseClass` groups. Difficulty-enemy text, the redirected enemy nickname, the one-shot resurrection line and all post-resurrection `I dun wan it` / `She muh queen` replacements carry their dynamic RU/EN bodies directly through `PhrasePayload`. `localization.en.json` contains Jon's canonical character/passive presentation entries, while the hidden resurrection description remains empty and no pre-trigger widget copy reveals it (`JonSnow.cs`; `CharactersPhrases.cs`; `PlayerCard.vue`).
