...
- Character flavor has hundreds of historical Russian quips. Exact/adapted text is used when catalogued; otherwise `PhraseForUser` selects the shared `phraseFallbacks` adaptation so English never depends on understanding the Russian joke. Older mixed snapshots with an already-translated passive title are normalized through the same catalog (`GameLocalization.cs:82-142`). `tools/audit-localization.sh:13-78` extracts all 132 constructed `PhraseClass` identifiers and requires both an English display name and a fallback for every Cyrillic one. Russian retains the original full phrase pool and no `PassiveLogRus` entry is changed.
