...
- Geralt hints use the same paired generation contract in one Haiku request. Valid generated adaptations—or a paired static fallback on any failure—are embedded into one `PhraseV2` personal-log record, making arbitrary AI prose switchable in live web, Discord and replays without a client lookup (`ClaudeHaikuService.cs:34-108`; `CP:4767-4802`).
