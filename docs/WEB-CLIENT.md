...
- **Server events → store** (handlers assigned in `useGameStore.connect()`, `game.ts:144-318`): `GameState` (`signalr.ts:1011`) → state + side effects (§6); `LobbyState` (`signalr.ts:1015`); `ActionResult` (`signalr.ts:1019`) → `lastAction`, failure shows `errorMessage` for 3 s (`game.ts:247-258`); `GameEvent` (`signalr.ts:1023`) → `lastEvent`, eventType GameStory fills `gameStory` (`game.ts:260-268`); `Error` (`signalr.ts:1027`); `Authenticated` (`signalr.ts:1032`) → `isAuthenticated`, `accountPlayerType`, `lastPlayedCharacter` (`game.ts:274-282`); `WebAccountCreated` (`signalr.ts:1036`); `GameCreated`/`GameJoined` (`signalr.ts:1040-1046`) → navigation is done by the Lobby page; `BlackjackState` (`signalr.ts:1048`); `QuestState` (`signalr.ts:1052`); `LootBoxOpened` (`signalr.ts:1056`); `AchievementBoard` (`signalr.ts:1060`); `CharacterList` (`signalr.ts:1064`); `BattleshipLobby`/`BattleshipState`/`BattleshipGameCreated`/`BattleshipGameJoined`/`BattleshipEvent`/`ShipCatalog` (`signalr.ts:1068-1090`) → battleship store.
