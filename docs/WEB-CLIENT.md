...
- **Top bar** (`App.vue:155-186`): nav Lobby / `Морской Бой - minigame` / Home / localized Achievements (`App.vue:161-166`), connection dot bound to `store.isConnected` (`App.vue:169-172`), user label, Logout (clears all three localStorage keys, `App.vue:105-110`), theme dropdown (`App.vue:177-184`).
...
Two login paths in `LoginProcess.vue`: a numeric **Discord User ID** input validated `/^\d+$/` with a "Copy User ID via Developer Mode" hint (`LoginProcess.vue:20-29`, validation `LoginProcess.vue:74`), or a **web username** (max 32 chars) that emits `webLogin` (`LoginProcess.vue:44` `LoginProcess.vue:65` `LoginProcess.vue:83`). `App.vue` then connects + authenticates and persists `discordId` (`App.vue:69-77`) or lets the server mint a web account via `registerWebAccount` (`App.vue:93-99`) — the store saves `kotgh_web_id`/`kotgh_web_username` when `WebAccountCreated` arrives (`game.ts:307-315`). `restoreWebSession` re-authenticates from those keys (`game.ts:681-695`). There is no password — see the trust model in WEB-BACKEND.md §2.
...
- **Game** (`Game.vue`, the main view): existing three-column play surface and action flows are unchanged. At finish, the game-over podium retains priority; queued `AchievementPopup` celebrations begin only after that overlay closes, so the result and reward moment do not compete (`Game.vue:1481-1485`). On every authenticated **non-game route**, `App.vue` is the global recovery host for the same queue (`App.vue:13-15,202-206`); authentication requests the board and the store reconstructs full queued cards (`game.ts:246-259,296-315`). Dismissing either host acknowledges the server queue.
