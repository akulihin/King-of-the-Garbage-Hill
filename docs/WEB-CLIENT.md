...
- **Login gate**: until authenticated, renders `LoginProcess` (`App.vue:125-132`), then a `LoginSuccess` splash (`App.vue:134-141`), then top bar + RouterView (`App.vue:143-195`).
...
- **Top bar** (`App.vue:145-177`): nav Lobby / `Морской Бой - minigame` / Home / localized Achievements (`App.vue:152-157`), connection dot bound to `store.isConnected` (`App.vue:159-163`), user label, Logout (clears all three localStorage keys, `App.vue:86-96`), theme dropdown (`App.vue:168-175`).
- **Themes**: dark by default; five alternates applied as a `data-theme` attribute persisted in localStorage `kotgh_theme` (`App.vue:22-32`) — blood, neon, forest, dark-light, siri, defined in assets/base.css; `App.vue:203-261` maps the palette onto semantic vars (--bg-*, --text-*, --accent-gold, glows, easing).
- **Error toast** bound to `store.errorMessage` (`App.vue:179-184`).
...
Two login paths in `LoginProcess.vue`: a numeric **Discord User ID** input validated `/^\d+$/` with a "Copy User ID via Developer Mode" hint (`LoginProcess.vue:20-29`, validation `LoginProcess.vue:74`), or a **web username** (max 32 chars) that emits `webLogin` (`LoginProcess.vue:44` `LoginProcess.vue:65` `LoginProcess.vue:83`). `App.vue` then connects + authenticates and persists `discordId` (`App.vue:53-60`) or lets the server mint a web account via `registerWebAccount` (`App.vue:83-88`) — the store saves `kotgh_web_id`/`kotgh_web_username` when `WebAccountCreated` arrives (`game.ts:307-315`). `restoreWebSession` re-authenticates from those keys (`game.ts:681-695`). There is no password — see the trust model in WEB-BACKEND.md §2.

## 4. SignalR client (`signalr.ts`)

Singleton `signalrService` (`signalr.ts:1508`); hub URL from VITE_SIGNALR_HUB, default /gamehub (`signalr.ts:3`).

- **Lifecycle**: `connect()` builds the connection `withAutomaticReconnect` at 0/1s/2s/5s/10s/30s (`signalr.ts:1053-1060`), registers every handler including reward events (`signalr.ts:1062-1146`), tracks state via `onreconnecting`/`onreconnected`/`onclose` (`signalr.ts:1148-1168`); **on reconnect it re-authenticates with the remembered ID and re-joins the remembered game** (`signalr.ts:1154-1162`). Authentication then requests the achievement board, so queued celebrations are recovered (`game.ts:296-315`).
- **Server events → store** (handlers assigned in `useGameStore.connect()`, `game.ts:144-344`): quest state restores an unacknowledged loot result; the achievement board rehydrates full queued cards after refresh/reconnect; plus DooM Guy `DoomFortressState` and the battleship event family (`game.ts:235-259,328-334`).
- **Invoke wrappers** mirror the hub 1:1 (WEB-BACKEND.md §4), including `doomRoll`/`doomChainsaw` (`signalr.ts:1296-1302`) and account-level `requestDoomFortress`/`equipDoomModule` (`signalr.ts:1408-1414`).
...
- State refs (`game.ts:57-85`): identity/game/meta state plus quest state, resumable `lootBoxResult`, achievement board/full celebration queue/loading/error state, character list and `doomFortressState`; the DooM action wrappers and Fortress state remain in the same store.
...
- `connect()` (`game.ts:144-344`) wires every SignalR callback; the `GameState` handler additionally: diffs own stats to fire level-up/stat-max sounds and class-change stingers (`game.ts:165-214`), **auto-joins Blackjack when newly dead to Kira** (`game.ts:216-219`), and captures full finish-time achievement entries once per game (`game.ts:221-228`). Quest/board callbacks restore server-held unacknowledged loot/achievement experiences (`game.ts:235-259`).
...
- Reward wrappers keep acknowledgement explicit: `openLootBox` waits for the server result; `acknowledgeLootBox` waits for server acknowledgement then refreshes inventory, while `clearLootBoxResult` removes only the matching local opening after success. Achievement dismissal snapshots the IDs actually shown, clears the local celebration, and sends those IDs through `AcknowledgeAchievements`; a later concurrent unlock remains queued (`game.ts:615-675`; hub wrapper `signalr.ts:1404-1406`). `ClearNewAchievements` remains only as a legacy-compatible wrapper (`signalr.ts:1400-1402`). Logout hard-resets all account-scoped achievement/loot state before and after disconnect (`game.ts:346-393`).
...
- **Lobby** (`Lobby.vue`): polls `refreshLobby` every 3 s while connected (`Lobby.vue:126-136`) and requests both reward states on mount. Its persistent Rewards hub shows live ZBS, pending box inventory, Rare+ distance/base Legendary chance, achievement completion/new badge and current-catalog reward totals (`Lobby.vue:210-320`). Opening immediately presents the staged loot modal; a result restored by the quest-state request reopens the same reveal. Continue/Open another keep the modal busy and visible until acknowledgement succeeds, then clear only that `openingId`; failures stay actionable in the modal (`Lobby.vue:27-112`; `Lobby.vue:369-382`). The remaining Daily Quests, Active Games, recent replays, How to Play and admin game controls are unchanged.
...
- **Game** (`Game.vue`, the main view): existing three-column play surface and action flows are unchanged. At finish, the game-over podium retains priority; queued `AchievementPopup` celebrations begin only after that overlay closes, so the result and reward moment do not compete (`Game.vue:1481-1485`). On every authenticated **non-game route**, `App.vue` is the global recovery host for the same queue (`App.vue:13-15,190-194`); authentication requests the board and the store reconstructs full queued cards (`game.ts:246-259,296-315`). Dismissing either host acknowledges the server queue.
...
| `LootBox.vue` | server-determined staged opening/reveal, rarity audio/VFX, exact reward and post-open wallet/inventory/pity, base-odds disclosure and pity-upgrade badge | Continue acknowledges; Open another acknowledges then opens next; saving/error stays visible (`LootBox.vue:35-201,213-380`) | |
...
| `AchievementPopup.vue` | sequential rarity celebration with sound/particles, related characters and explicit ZBS/box rewards; focus-isolated ARIA dialog with Escape/Skip all, focus restoration and reduced-motion fallback | Next / Continue / Skip all → dismiss queue (`AchievementPopup.vue:20-173,176-270`) | |
...
- Achievement and loot surfaces reuse the established UI sound palette through rarity-aware helpers: each unlock/reveal rises in intensity by rarity, while loot has a separate opening cue before its reveal (`sound.ts:450-469`). Both reward components honor `prefers-reduced-motion` without removing reward information (`AchievementPopup.vue:422-436`; `LootBox.vue:498-510`).
...
Generated game stories carry paired RU/EN blocks in one artifact; `html[lang]` selects the visible block (`App.vue:301-304`). Development builds warn on Cyrillic left in an English-rendered DOM node (see `Web/VueClient/src/i18n.ts`).

## 14. Known gaps & dead surface

- **ARAM pick phase has no web UI**: the contract carries isAramPickPhase and the store exposes `aramReroll`/`aramConfirm` (`game.ts:520-527`), but no component calls them — a web-preferring player in an ARAM game has no pick surface (finding **m24** in AUDIT-FINDINGS.md).
