...
- **Server events → store** (handlers assigned in `useGameStore.connect()`, `game.ts:144-318`): the existing game/lobby/meta events plus DooM Guy `DoomFortressState` (`signalr.ts:1025, 1104-1106`) → `doomFortressState` (`game.ts:308-310`), and the battleship event family.
- **Invoke wrappers** mirror the hub 1:1 (WEB-BACKEND.md §4), including `doomRoll`/`doomChainsaw` (`signalr.ts:1265-1271`) and account-level `requestDoomFortress`/`equipDoomModule` (`signalr.ts:1369-1375`).
...
| `PassiveAbilityStates` | owner-only widget contract, including `doomGuy` and `eren`; Eren carries Rage/loss progress, hatred marks, Rumbling place and monotonic Tatake/Titan audio serials | `signalr.ts:129-174, 237-255` |
