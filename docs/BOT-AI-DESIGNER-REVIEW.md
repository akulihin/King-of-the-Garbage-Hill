...
| AI-TURN-06 | Post-round-10 and locked `Мадара` states cannot take a normal action. | Round `> 10` submits the no-target action. `Мадара` is action-locked on round 8 or while sealed. L1 keeps its historical scripted exact round-8 prediction; L2/L3 must use legal evidence. |
...
| ID | Designer view | Technical detail |
|---|---|---|
| AI-ATTACK-ELIGIBLE | Only selectable living opponents are considered; the bot does not attack itself or its Naruto pair. Public round-10 bans and the visible Darksci quit line remove targets. | `HandleBotAttack` builds the legal `Nanobot` list, uses the viewer's `🚫` annotation for the round-10 `Тигр` ban, and parses only sanitized visible logs for `Нахуй эту игру`. |
...
| AI-PRED-EXACT | Exact public or owner-visible reveals override guesses permanently. | `Толя` public reveal; `Коммуникация` when one public name maps to one Pink-Ward id; owner `Сверхразум`; Naruto sibling. Stored at confidence 100 and protected from weaker replacement. |
...
| AI-PRED-MONSTER | If the best hypothesis is `Монстр без имени`, remember the belief but submit no prediction. | That character cannot be scored and a wrong submission can feed its punish. `RecordFairPredictionChoice` keeps evidence but removes the UI prediction row. |
...
|---|---|---|
| AI-PLAN-DOPA | Dopa | Records the actual `Законодатель меты` choice: `Стомп`, `Фарм`, `Доминация`, or `Роум`; a random fallback applies it only if setup did not. |
...
| AI-CHAR-ITACHI | `Итачи` | Stack crows, avoid wasting the active Tsukuyomi target, exploit estimated Speed, and pressure leaders when charged. | Active Tsukuyomi target `-25`; crows `+5` each plus `+10` at 3; Speed advantage and adjacency bonuses; charge 2+ versus top two `+8`. Always attacks. |
...
| AI-CHAR-SAKURA | Sakura | Use the universal target policy; defend a late top-three position only when earlier public rounds show pressure. | From round 8 at place 1–3 with historical incoming attacks, prefer block; otherwise prefer attack. Sakura prediction remains forbidden. |
| AI-CHAR-DEFAULT | DooM Guy, `Наруто`, `Мадара`, `unknown_bug`, and any unlisted/transform-only entry | Use universal targeting unless the engine/character owns a separate forced action. | Neutral fair block plan by default. Naruto pair is excluded from legal targets. `Мадара` turn locks are handled in the shared pipeline. `unknown_bug` identity remains masked/unguessable. |
...
| AI-L1-MADARA-EREN | Receives the historical exact `Мадара` prediction and scripted `Гул земли` targeting. | L2/L3 must associate the public warning with a player through legal evidence; no server-located target. |
