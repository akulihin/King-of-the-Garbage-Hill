...
| AI-INFO-SCRIPT | Three explicit designer events may use information outside ordinary inference. | They create coordinated set pieces rather than ordinary deduction. | Round-8 Клоны Сусано reveals Madara to strict bots with prediction sheets; round-10 exact Eren-above priority precedes Monster inference; a Naruto may follow another living Naruto's submitted target. The Monster ranker and every ordinary prediction row stay inside the viewer boundary and receive no real Monster seat. |
...
| AI-TURN-09 | Global designer reactions are independent of AI version and override ordinary behavior. | Prediction mandates enter through `ApplyGlobalDesignerPredictionRules`; attack mandates enter through `TryApplyGlobalDesignerActionRules` after genuine unable-to-act and legal-target filtering but before the L0/Legacy/Legacy+/V2/V3 dispatch. Current priority is the named Madara challenge → round-10 Eren-above → round-10 Monster inference → Нечто. New cross-roster mandates must extend this overlay rather than an individual version branch (M247). |
...
| AI-ATTACK-R10 | On round 10, every strict bot below a selectable Eren attacks him; every other bot attacks the player it independently considers most likely to be `Монстр без имени`. | `TryForceRoundTenDesignerAttack` prepends exact Eren targets above the acting bot, then appends `RankRoundTenMonsterSuspects`. The ranker scores every selectable target as Monster against its best admissible non-Monster score, demotes exact non-Monster reveals and randomizes equal hypotheses without receiving the real Monster seat. Rejected submissions fall through the combined list before ordinary target/Block policy. Dopa then submits the next distinct target required by Макро (M189/M247). |
...
| AI-PRED-ALL | Every prediction-capable strict L0–L4 bot must submit one admissible hypothesis for every admissible enemy, regardless of confidence. | The common solver always selects a highest-scoring non-Monster catalogue value. L0–L2 keep their independent 25% low-confidence row; L3/Legacy+ may fall to 10% and use that weight in later roster reasoning. Exact admissible evidence remains fixed. Ordinary inference never asks which seat really is Monster or Eren. |
...
| AI-PRED-L3 | L3 solves the lineup like a logic puzzle, but treats an unconfirmed answer as a weighted hypothesis rather than a fact. | L3 ranks every target/candidate pair, chooses the most constrained seat first and stores 10–95 confidence. A fallible assigned name stays available for other rows with a quadratic confidence penalty; exact reveals and `Кира` still reserve their identity absolutely. L3 additionally uses longer-lived own-fight class evidence, public strict-bot roll prior, attack/defense tendencies, public speaker tells, place-six clues, and prior consistency. |
| AI-PRED-L4 | Legacy+ and L3 fill every ordinary enemy row with confidence-weighted public-roster inference. | Exact admissible reveals stay fixed at 100% and hard-reserve their identity. Every fallible choice receives 10–95% confidence. Its name remains available for later rows but receives `floor(80 × confidence² / 10,000)` reuse penalty; repeated weak reservations combine gradually. Weak priors can therefore duplicate rather than steer the remaining sheet down a false path, while strong evidence behaves almost like an all-different reservation. `Кира` remains a hard one-row exception. |
...
| AI-CHAR-EREN | `Эрен Йегер` | As Eren, value visible fire marks. Other bots below Eren must attack him on round 10. | Own pilot adds visible fire count `×3`. The global overlay receives Eren's exact selectable seat as an explicit designer exception and places it before the Monster ranking for every Legacy/Legacy+/V2/V3 bot below him (`TryForceRoundTenDesignerAttack`; M247). |
...
| AI-L3-PREDICT | Uses public roll/action/defense patterns and longer-lived evidence, processes the strongest hypotheses first, then applies a quadratic confidence-weighted reuse penalty. A weak seat barely reserves its guessed name; strong evidence nearly does. | Guesses seats independently; weak seats use a tier prior or prior guess. |
...
| AI-L1-MADARA-EREN | Shares the exact Madara prediction and the global round-10 Eren/Monster overlay. | Naruto/Sakura/Itachi additionally make the ordinary Madara attack. Exact selectable Eren-above priority is the named designer exception; otherwise the Monster hunt receives no real Monster seat (M189/M247). |
...
| AI-L4-PREDICT | Infer all five enemies like the advanced fair solver. | Exact admissible earned/public/owner reveals stay 100%. Other rows use public evidence, 10–95% confidence, hard public incompatibilities and soft name reservations whose quadratic penalty tends to zero with weak confidence. A low-weight guess therefore cannot fully displace a likely name from later enemies. The real Monster seat receives no target-specific override because the solver never knows which seat it is. |
...
5. **Information test cases:** specifically stage an opponent choosing block late, then confirm an L2/L3 action is unchanged until that block becomes a resolved visible result. Repeat with hidden identity, hidden score, private logs, hidden fights and forced hidden rosters; changing only the real Monster seat must not feed the shared solver directly. Test exact Eren separately as the authored round-10 overlay exception.
6. **Inference test cases:** verify every prediction-capable L0–L4 strict bot fills every admissible enemy row with a non-Monster value, including at low confidence. For L3/L4, compare weak/medium/strong evidence: the same-name reuse penalty must be 0/5/20/45/72 at confidence 10/25/50/75/95, repeated weak reservations must accumulate gradually, and exact admissible reveals plus the one-Kira rule must remain hard. Lock the sheet after round 8, then verify late processing fills only missing rows while using retained confidence only as a soft reservation. For round 10, verify relative Monster-vs-best-alternative ordering, unresolved-before-exact-non-Monster ordering, randomized exact ties, rejection retry and Dopa's distinct second target. On a secret/Monster Макро target, a strict bot must preserve its fair hypothesis or, when none exists, use the deterministic highest-Tier/name-tied public prior; only the human fallback remains random. Separately retain the exact Madara and Naruto-focus scripted rules.
