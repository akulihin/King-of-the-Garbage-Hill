...
Шэн's successful leap is also a one-round random-target override. It replaces Rick (when present) or one ordinary Спартанец mark, redirects Kira's L/Weedwick's hunter priority to the holder, and consolidates every currently banked Геральт contract onto the holder. Original targets are not mutated and resume automatically next round; copied Шэн uses the copy holder as the magnet (`Salldorum.cs` `FindRandomTargetMagnet`/`IsRedirectedRandomTarget`/`TakeGeraltContractCount`).
