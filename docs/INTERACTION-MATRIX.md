...
|---|---|---|---|---|---|---|
| Dead player | ✓ excluded | ✓ excluded | ✓ targets exclude dead | n/a (dead don't fight) | ✓ excluded | ✓ excluded |
| Тигр round-10 ban | ✓ carve-out `CIR:1293` | ✓ carve-out (M11 fixed, `CIR:1339-1340`) | n/a | targeting already blocked (`GR:702-707`) | not special-cased; ordinary block/skip still applies | ✓ excluded from fan-out `DM:459-470` |
...
Гарем's row applies only when its cooldown is 0 and the submitted Block is converted. For the next two turns after a use, that Block remains ordinary, so every forced-fight interaction follows the normal Block row instead (`Naruto.cs` `HaremCooldownTurns`/`ResolveHaremQueues`; CP `HandleEndOfRound`).
