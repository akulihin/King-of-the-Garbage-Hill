...
Forced-fight sources: **Монстр** two-turn no-escape (`CP:1066-1073`; `CIR:1371-1393`), **Шэн** attack-bound leap + below-position pull (`Salldorum.cs` `ResolveShenDashes`; `CIR:1433-1445`), **Штормяк** taunt (`CIR:1327-1361`), **Aggress** self auto-attack (`CIR:1298-1310`), **Геральт** contract multi-fight injection (`DM:315-370`), DooM Guy **BFG** wave injection (`DM:855-881`) and **Рельса** one-side injection (`DM:445-491`).
...
| Тигр round-10 ban | ✓ carve-out `CIR:1293` | direct attack targeting is rejected; below-position pull also excludes him (`Salldorum.cs` `ResolveShenDashes`) | ✓ carve-out (M11 fixed, `CIR:1339-1340`) | n/a | targeting already blocked (`GR:702-707`) | not special-cased; ordinary block/skip still applies | ✓ excluded from fan-out `DM:459-470` |
...
| Ziggurat lock | position only — fights unaffected | the leap, current-round random-target magnet and pull are cancelled if inserting the holder ahead of the target would shift any locked occupant | position only | n/a | n/a | position only — wave follows current board order | position only — side is captured from current order |
...
| Active Вечное Цукуеми round 10 | erased by total Skip | charge, leap, magnet and pull are not spent/applied | erased | suppressed | no contract expansion | no wave can start | no fan-out | the authoritative queue is cleared before combat; only non-combat/post-round passive settlement remains (`Madara.PrepareEternalTsukuyomiRound`; `DM:251-405`) |
| Мадара sealed | all queued targets sanitized | all queued targets sanitized after the pull | all queued targets sanitized | cannot act | pre-fight targets sanitized | ✓ excluded | ✓ excluded `DM:464` | `Madara.SanitizeSealedActions` runs after forced injections; direct targeting says `Игрок запечатан` |

Шэн's successful leap is also a one-round random-target override. It replaces Rick (when present) or one ordinary Спартанец mark, redirects Kira's L/Weedwick's hunter priority and any same-round Великий Комментатор reveal to the holder, and consolidates every currently banked Геральт contract onto the holder. Original targets are not mutated and resume automatically next round; copied Шэн uses the copy holder as the magnet (`Salldorum.cs` `FindRandomTargetMagnet`/`IsRedirectedRandomTarget`/`TakeGeraltContractCount`).
...
| Тигр ban (round 10) | ✓ swap suppressed `DM:1304-1306` | n/a | n/a | n/a | cannot be the direct target; never joins the below-position pull (M11 preserved) | inactive after round 8 | clone-only move; banned Тигр remains above both clones |
