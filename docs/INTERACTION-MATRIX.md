...
The Гарем row applies only when its cooldown is 0 and the submitted Block is converted. For the next two turns after a use, that Block remains ordinary, so every forced-fight interaction follows the normal Block row instead (`Naruto.cs` `HaremCooldownTurns`/`ResolveHaremQueues`; CP:3752-3765).
