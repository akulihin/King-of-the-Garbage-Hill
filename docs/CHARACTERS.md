...
- Bots/Discord/web: bots understand the immunity bypass/full doubled Harm (`BotsBehavior.cs:1046-1072`); the owner alone sees 🦸 on marked enemy rows (web `GameStateMapper.cs:137-145,345-351` / `PlayerCard.vue:617-625`; Discord `GameUpdateMess.cs:697-719`). Targets receive no sup notification and the owner widget no longer lists their nicknames (m27 fixed).
...
- **Audio/UI** — owner widget exposes gained Rage, losses toward the Rumbling gate, Titan state, hatred marks and resolved Rumbling place (`GameStateDto.cs:775-793`; `GameStateMapper.cs:349-370`; `PlayerCard.vue:1205-1224`). Tatake/Titan sounds are owner-only serial events; the Rumbling warning and `eren_game_win_theme.mp3` are global (`Game.vue:180-228`; `sound.ts:947-965`).
