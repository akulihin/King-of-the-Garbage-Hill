...
Batch-audited order: TheBoys, Стая Гоблинов, Рик Санчез, Сайтама, Тигр, Итачи (recent reworks) · Кратос, Кира, Монстр без имени, Продавец, Dopa, Salldorum · Геральт, Котики, Toxic Mate, Napoleon, Таинственный Суппорт, Осьминожка · DeepList, mylorik, Глеб, LeCrisp, Толя, HardKitty · Sirinoks, Злой Школьник, AWDKA, Darksci, Братишка, Загадочный Спартанец · Вампур, Краборак, Weedwick, Молодой Глеб, Sakura, Баг
...
- **Francie** — orders: first order at game start (`CP:283-300`), re-issued on rounds 4 and 7; window 3 rounds; completing = +1 bonus, expiring (checked on rounds 4/7/10) = −1 bonus (`CP:5622-5652`). Хим.оружие (level ≥1): +chemLevel bonus points on a win where TheBoys was **not** TooGood/TooStronk vs the victim (`CP:3315-3324`). ⚠ the design note's "+1 per enemy difficulty tier" scaling is not implemented — flat chemLevel.
- **Butcher** — sup marks assigned each round after sorting (`CP:5864-5898`): superheroes always marked; +2 rotating marks (prefer current Мишень class; never the Francie order target; none on round 1). Attacking a marked sup: +10 Skill (+20 under СуперМудень), **+1 bonus point on a win** (+2 SD) (`CP:3350-3368`). ⚠ spec says the point is for a **Drop** ("Скинуть"), code pays it on any win. Poker multiplies fight skill: `SkillFightMultiplier = 1 + poker` (×2 under SD) (`CP:1530-1538`) and repeats Harm per poker (SD doubles). Under SD, each Harm that FULLY breaks the victim's Strength pool (underflow→reset→drop via `HandleDrop` — not a mere −1; Int/Psyche breaks don't count; place-6 victims register no drops) grants one more Harm, looping while new Str-drops land, safety 50 (`DoomsdayMachine.cs:839-868`, `CharacterClass.cs:299-323`).
- **Kimiko** — while active, TheBoys take **no Harm** (`CharacterClass.cs:202-210`). Defense: reduces attacker's Justice by RegenLevel for the fight (`CP:659-676`); +10 Skill on defense win, +20 on successful block (`CP:735-741`). Defensive loss disables her next round (`CP:923-930`); leveling her instantly re-enables (`GameReactions.cs:946-947`). **Живое Оружие** (x4): never disabled, steals attackers' *real* Justice after every defensive interaction regardless of outcome (`CP:744-755`).
- **M.M.** — team psyche: +1 if zero losses this round, −1 + психует if lost every fight (`CP:3415-3431`). Компромат: after a Psyche upgrade the next *actual* fight on attack files kompromat + a character hint (`CP:3379-3400`); round 8: if every kompromat target is correctly predicted, +5 Мораль each (`CP:3434-3447`); game end: prediction bonus ×kompromat count (`CheckIfReady.cs:310-338`). **Оковы Правосудия** (x4): steals all kompromat targets' current Мораль, permanently blocks their moral gains (`BlockMoralGain`), M.M. becomes calm = immune to psyche loss (`GameReactions.cs:965-986`, `GamePlayerBridgeClass.cs:97-100`).
- **Смертельный вирус** (Francie x4): next attack infects; spreads on any real fight between carrier and non-carrier (block/skip don't spread; Француз immune) (`CP:3327-3346, 2345-2370`); game end: −2 bonus per infected → +2 each to Francie (`CheckIfReady.cs:341-360`).
...
- **Гоблины** — growth: +1+Hobs auto per round (`CP:6118-6120`); attack win: +2×(1+Hobs), +1 if enemy TooGood, +2 if TooStronk (`CP:2260-2271`). Any loss: −(10 + 0.5·R²/3)% ±5% per TooGood/TooStronk, min 1 goblin left, min 1 dead (`CP:2272-2282, 890-903`).
...
- **Гоблины тупые, но не идиоты** (Ziggurat) — Block arms a build; resolved after sorting (`CP:6137-6207`): needs ≥1 of each goblin type **and score ≥3** (⚠ undocumented cost gate); costs −3 bonus + 1 permanent Worker deduction (⚠ design said "WorkerRate goblins die"); builds at current place (once per place, max 6); learns a random `Standalone: true` passive from the last attacked enemy (no duplicates). Standing on any built position: position locked for the round (immune to Тигр-топ/Portal-Gun/Шэн/Drops/Storm — enforced in `DoomsdayMachine.cs:1286-1499`) +1 next-round Justice +5 Мораль per round. ⚠ "Round-10 ziggurat at place 1 = win" is only a log line at the *start* of round 10 (finding M1).
...
- **Гигантские бобы** — Int uncapped (`CharacterClass.cs:1206`). Each level-up: +BaseInt tracking, ingredients appear on up to 3 enemies without one (`GameReactions.cs:1141-1202`). Any win vs an ingredient carrier (attack `CP:2072-2088` or defense `CP:841-857`): +1 stack, −1 Str/Speed/Psyche, Int = BaseInt × stacks.
- **Most wanted** — no case of its own; every "random mark" passive force-targets Rick: Спартанец marks (`CP:118-128, 3874-3877`), Kira's L (`CP:233-236`), Сверхразум discovery (`CP:5269+`), Глеб tea odds (`CP:5130`), Weedwick's hunter senses Rick regardless of Justice (`CP:1118-1124`), and headhunters follow Rick through portal swaps (`CP:2108-2123`).
- **Огурчик Рик** — Block converts to pickle: 2 turns, not IsSkip (stays attackable), attackers **cannot win** (`DoomsdayMachine.cs:253-267`, `CP:598-604`). Player keeps control: skip button shown, may level up or fire the Portal Gun (choosing a target auto-confirms the skip) (`CP:5425-5445`, `GameReactions.cs:659-662`, `CheckIfReady.cs:1087-1090` excludes pickle from auto-move). If never attacked while pickled: +1 extra forced-skip penalty turn (`CP:4072-4079`).
- **Портальная пушка** — invented at Int ≥ 30 (checked at level-up and end of round), +1 charge per level-up once invented (`GameReactions.cs:1155-1165`, `CP:4046-4053`). With a charge, his attack ignores block/skip and can't lose (`CP:1358-1366`); on the win: charge spent, positions swap **mid-round** and remaining attackers are exchanged between Rick and the target (`CP:2091-2126`); at end of round the leaderboard swap is enforced (Ziggurat-protected; `DoomsdayMachine.cs:1331-1356`) and the round's regular points are **×2** ("две мульти-вселенные", disabled by Толя's Подсчет; `CP:4054-4069`).
...
- **Неприметность** — serious targets = top-2 enemies by `GetSkill()`, recomputed every round end (`CP:4027-4036`; round 1 uses game-start skills ⚠ effectively arbitrary). Rounds 1–9, defending vs a non-serious attacker: Saitama can't win; the foregone win point (round-multiplied) + underdog moral are **banked**, no upfront loss (`CP:567-596` — the fix from saitama_update). Attacking a target that a co-attacker also hit: his +1 is deferred to the recipient (−1 regular now, banked) along with moral (`CP:2037-2069`). Solo wins vs Мишень-class: см. На мели.
- **На мели** — win vs your Мишень-class target: +1 bonus; if nobody else attacked the victim: +1 regular and the fight is hidden from non-admin logs (`CP:2007-2034`).
- **Ищет достойного противника** — after round 10 (`RoundNo == 11`, `CP:4807-4856`): if Saitama's round-10 attack beat the then-#1, he reclaims the whole ledger (+bonus to himself, −banked amount from each recipient), restores deferred moral and force-converts it to score with a manual flush. On round 10 Неприметность is off (`CP:568, 2040`), so nothing new defers.
...
- **Стримснайпят и банят и банят и банят** — entering round 10: forced skip, stats set Psyche 0 / Int 0 / Str 10, "ЕБАННЫЕ БАНЫ" global (`CP:4936-4951`); can't be targeted (`GameReactions.cs:702-707`), Монстр's no-escape and Тигр-топ swap carve-outs respect the ban (`CheckIfReady.cs:1270`, `DoomsdayMachine.cs:1304-1306`) — but Шэн and Штормяк don't (finding M11). AutoMoveTimes ≥ 9 winner is renamed "НейроБот" with a ban-specific allowance (`CheckIfReady.cs:514-516`).
...
- **Похищение души** — `ClassSkillMultiplier = 2` from game start (`CP:73-75`): class skill perks pay double (Сильный +8/win, etc.). ×4 during the resurrection event (`CP:2528`).
- **Охота на богов** — vs his Мишень-class target: fight-skill ×2 (×4 during event) (`CP:1077-1086`), reset after each fight (`CP:2533-2535`).
- **Возвращение из мертвых** — losing any round-10 fight starts the event (`CP:2521-2529`): global warning, Kratos music (5 rounds), class mult ×4. Extra rounds 11–16: everyone else is forced to auto-block (`CheckIfReady.cs:1020-1026`), Kratos ignores blocks/skips (`CP:1068-1074`); every enemy he defeats **dies** (Goblins immune) and drops off (`CP:1694-1714`); a Kratos loss = his final death (`CP:2502-2518`); event ends at round ≥16 or when 5 players are dead (`CP:3454-3468`, `DoomsdayMachine.cs:1249-1255`; hard cap `RoundNo ≥ 20`).
- **Боги мне не указ** — one-time resurrection if killed by Kira, +228 Skill (`CP:5471-5480`).
...
- **Гений** — +4 Int at start (`CP:219-221`), −1 Int per Death-Note kill (`CP:4121`). Also strips character-revealing lines from Kira's own global-log view (`GameStateMapper.cs:1124`, `KiraHiddenLogSnippets`).
- **Тетрадь смерти** — replaces predictions (auto-confirmed, `CheckIfReady.cs:1339-1340`); one name per round via web/bot action; resolved at end of round (`CP:4082-4160`): 15% "писал на стекле" fizzle; correct → target dies (DeathSource "Kira", Goblins immune), Kira +2 regular (+4 if it was L), wrong → that target locked forever.
- **Глаза бога смерти** — activation costs 25 Мораль (`WebGameService.cs:747-764`; bots `BotsBehavior.cs:438`); next attack reveals the target's character; **not consumed** on L or Монстр (`CP:999-1021`).
- **L** — random enemy (prefers humans; Most wanted forces Rick) (`CP:223-240`). +5 Мораль per round without fighting L (`CP:4163-4180`). From round 8: if L has predicted "Кира" on him, Kira is arrested — dies, −500 points (`CP:4764-4805`).
...
- **Монстр** — anyone he attacked can't block/skip next round (`CP:1690-1692` set, `CheckIfReady.cs:1266-1289` enforcement — random forced attack; round-10 Тигр-ban carve-out; cleared each round `CP:5616-5619`). +1 regular per any death (`CP:1709-1714, 4112-4117, 2512-2517`), +1 bonus per any Drop (`CharacterClass.cs:188-192`).
- **Близнец** — blocking steals the attacker's entire real Justice + bonus points equal to it (`CP:905-920`); attacking a stat-twin (any one equal stat, FightCharacter values): −1 Psyche (`CP:1512-1527`).
- **Выдуманный персонаж** — can't be predicted (`CheckIfReady.cs:301`, `GameStateMapper.cs:54`) or named by Глаза бога смерти. Entering round 9: everyone *Monster* guessed correctly becomes a Johan pawn; +3 bonus if anyone has a guess on Monster (`CP:5565-5592`).
- **Пейзаж конца света** — start of round 10: global warning, Тэнма's hint self-erases after ~10s (`CP:5595-5613`). End of round 10: pawns who didn't block/skip die (+1 regular each to Monster); non-pawns who attacked Monster that round get **+7 regular** (×4 round multiplier!) **+10 bonus** (`CP:4394-4427`).

## Продавец Сомнительных Тактик — Tier 2, all stats 0

- **Впарить говна** — attack "sells" a tactic (cd 2): target +500 Skill for 4 rounds (excluded from siphon), target's next attack force-loses, after which the target marks *the enemy they lost to* as their personal "outplay" (+1 bonus per win over outplay targets while marked; each such win also counts into the Seller debt ledger `SellerTacticBonusEarned`) (`CP:1392-1415, 1640-1647, 2396-2408`). Mark expiry removes the granted skill and harvests everything the target *gained* while marked into the Секретный билд box (`CP:5484-5513`).
- **Закуп** — level-up gives +10 instead of +1 (`GameReactions.cs:1007-1011`).
- **Выгодная сделка** — each win by any marked player (or by DeepList — "Сомнительная тактика" holder counts as a tactics user, `CP:2416`) = 1 deal: +5 Мораль immediately (`CP:2410-2422`) and +1 bonus per deal at end of round (`CP:4222-4232`). Round 10, Seller wins an attack: steals ⌈debt/2⌉ bonus from the victim's tactic earnings (`CP:1677-1688`).
- **Большой куш** — 10%: an attacker who beats the Seller steals 2 bonus (`CP:2425-2436`).
- **Секретный билд** — hidden: all skill siphoned from marked players lands on the Seller on round 10 (`CP:5515-5536`, siphon plumbing `CharacterClass.cs:87, 994-995, 1025-1026`).
...
- **Пассивный импакт** — +1 bonus each round with ≥1 win (`CP:2129-2132, 4235-4242`).
- **Взгляд в будущее** — needs both targets: procs if one target attacked the other **or either blocked** (slightly wider than the description), +2 regular (+4 with Фарм) +50 Skill, cd 1 (`CP:4244-4271`).
- **Законодатель меты** — pick 1 of 4 at game start (web `WebGameService.cs:797-813`; bots random `CP:5555-5561`): Стомп +9 Str +99 Skill (then both passives removed, `CP:5822-5841`); Фарм doubles Взгляд; Доминация +20 Skill/win, victim −1 bonus, 33% −1 Psyche (`CP:2134-2143`); Роум wins vs non-adjacent: steal 1 bonus + 3 Мораль (`CP:2145-2159`).

## Salldorum — Tier 2, Int 6 / Str 1 / Speed 2 / Psyche 8

- **Шэн** — +1 charge per level-up (`GameReactions.cs:1168-1172`); activation (web) picks a leaderboard position: all players below it are forced to attack Salldorum (`CheckIfReady.cs:1184-1199`), and after sorting he *swaps into* that position (Ziggurat-protected) (`CP:6211-6236`). Combinable with block/skip/attack; deactivation refunds the charge (`WebGameService.cs:866-906`).
- **Очко** — +1 bonus when attacked from below (`CP:934-941`).
- **Временная капсула** — first Block buries the cola at the current position (`CheckIfReady.cs:1202-1214`); returning there ≥3 rounds later: +2 bonus + 5 Speed for one fight (pending → applied post-DeepCopy, `CP:5794-5814, 381-387, 978-984`); re-obtainable via history rewrite if he stood there that round (`WebGameService.cs:950-961`).
- **Великий летописец** — passive: ×3 fight skill vs whoever won most 3 rounds ago (ties → all of them) (`CP:2230-2258`); sees everyone's personal logs each round + 20% chance to corrupt a random line ("██…" or "Салдорум был здесь…") (`CP:4273-4305`). Active (once, before round 8): rewrite a past round — steals **1 flat bonus point** from each enemy who beat him then (⚠ design says 1×that-round's multiplier and redirect to Евреи if they stole the win), +2 Psyche +2 Justice (`WebGameService.cs:908-967`).
...
- **Минька** — +1 Мораль +10 Skill from every real fight, win or lose (`CP:3097-3113`); never deals Harm or fight-moral loss (`DoomsdayMachine.cs:750-806` isHarmless).
- **Штормяк** — blocking taunts a random enemy into attacking Котики as a forced extra action (once per unique enemy for the original; dead excluded; Котики immune to a *transferred* Storm's taunt) (`CheckIfReady.cs:1217-1251`); the taunt bypasses the block (fight happens, `DoomsdayMachine.cs:471-474`); if the taunted attacker loses: −1 Psyche and their top stat −1 → Котики +1 same stat (`CP:3117-3158`).
- **Кошачья засада** — attacking an enemy with no cat: deploys an off-cooldown cat 100% (50/50 if both ready); the cat's passive physically moves to the enemy (Storm also carries "Рандомное поведение") (`CP:3235-3289`). Re-attacking the carrier returns the cat (cd 2): Минька win → +2 bonus, +33 Skill × rounds she sat; Штормяк win → victim loses **half of their total score** (⚠ design says half of points earned *while the cat sat*) + психует (`CP:3161-3232`).
- **lvl-мяк** — level-up gives only +1 live Justice (`GameReactions.cs:897-904`, keyed on Name).
- **Рандомное поведение** — hidden, rides with Storm: each round picks a trick (fight 3/7, bite 1/7, vase 3/7 once) (`CP:5693-5758`): *fight* — Storm jumps into a random fight, ±5 weighing; if that flips the result the +1 win point goes to the Storm carrier (`DoomsdayMachine.cs:350-386, 639-668, 770-773, 903-904`); *bite* — random non-#1 player is position-locked for a round; if still there next round: +10 bonus to the carrier (`CP:5716-5735, 6241-6264`, locks in `DoomsdayMachine.cs:1269-1283, 1403-1450`); *vase* (once per game) — a vase falls on a random player at end of round: catch chance = Skill/3 % (never with ≤20 Skill); caught → +1 bonus + permanent vase immunity, dropped → −1 bonus and the vase chain spreads to both leaderboard neighbors next round (`CP:5737-5753, 4653-4704`).
...
- **INT** — +1 regular per loss (`CP:3050-3061`); "Ok. I'm trolling." on first loss; attack-wins score −1 (`DoomsdayMachine.cs:775-781`) ⚠ defense-wins still +1 (finding M4).
- **Get cancer** — first attack-win infects (victim can't gain Мораль) (`CP:3063-3084`); holder's attack-win passes it on (once/round); returning to Toxic Mate: +2×transfers bonus, then re-armable (`CP:2455-2496`).
- **Aggress** — can't block or skip, ever (button-blocked `GameReactions.cs:319-323`, force-cleared `DoomsdayMachine.cs:281-289`, `CP:1451-1458`, auto-attacks when idle `CheckIfReady.cs:1171-1182`); +1 regular +1 Justice when his attack fizzles on a block/skip (`CP:3086-3094`).
- **Tilted** — end of round: +1 bonus per *skipping* enemy (⚠ JSON says "когда кто-то психует"); +50 bonus if all five enemies blocked/skipped (log jokes "+20") (`CP:4320-4336`).

## Napoleon Wonnafcuk — Tier 5 (TeamModeOnly), Int 7 / Str 3 / Speed 3 / Psyche 7

- **Вступить в союз** — first attack designates the ally (they're told); afterwards attacking a target the ally also attacks: Napoleon can't lose, +3 Мораль (`CP:1433-1448`). Ally targets shown via leaderboard ⚔ icon.
- **Завоеватель** — win vs an enemy placed strictly between Napoleon and his ally: +1 bonus (`CP:2204-2227`).
...
- **Раскинуть щупальца** — +1 regular per new leaderboard place visited (rounds 2+) (`CP:5960-5970`).
- **Чернильная завеса** — defensive would-be-wins flip into attacker wins; the ledger records ±(round value): attacker (or the Еврей who stole the point) −1×mult, Octopus +1×mult (`DoomsdayMachine.cs:700-703`, `CP:6767-6835`). DeepList's first-fight-loss phase is exempt. Round 11: everyone's real score restored via bonus points (`CP:4859-4877`).
- **Неуязвимость** — attackers' Strength is 0 for the fight (`CP:423-425`); each *attack* Octopus loses: +1 counter → +1 bonus each at round 11 (`CP:1887-1889, 4871`).
- **Привет со дна** — Мораль interceptor: any non-button moral change becomes exactly +4, losses ignored (`CharacterClass.cs:1143-1153`); +1 bonus per block/skip event in the game each round (`CP:3699-3703`).

---

## DeepList — Tier 6, Int 10 / Str 5 / Speed 7 / Psyche 0

- **Сомнительная тактика** — must lose the first fight vs each unique enemy (attack `CP:1060-1066` and defense `CP:416-421` both set `IsAbleToWin=false`); the loss registers them as "known" (`CP:2548-2557`); afterwards +1 regular per win over them (`CP:2559+`). Suppresses Octopus ink and LeCrisp assassins while unmet (`CP:6772-6779, 489-494`).
- **Безумие** — 2(–3) scheduled rounds in 4–7 (`PassivesClass.cs:21`): all four stats random 0–10, skill multipliers ×4 ("квадра", `SetAnySkillMultiplier(3)`) (`CP:5311-5374`); restored next round with delta math; the Безумие flag makes `AddPsyche` skip the 0-floor — psyche can go negative afterwards ("энтропия") (`CharacterClass.cs:1295, 1344`).
- **Сверхразум** — 1(–3) scheduled discoveries in rounds 1–5: learns a random unknown enemy (Most-wanted Rick first) and auto-fills the prediction (`CP:5261-5307`).
- **Стёб** — second win vs a unique enemy: victim −1 Psyche (−2 for Злой Школьник; LeCrisp immune by name), +1 regular to DeepList; victims below 4 Psyche with Justice also lose 1 next-round Justice (LeCrisp immune) (`CP:2569-2624`).
...
- **Месть** — first loss to each enemy registers the grudge; later beating them (not the same round): +2 regular, +3 Мораль, +1 Psyche, once per enemy (`CP:2626-2654`).
- **Спарта** — the Block button is refused ("Спартанцы не капитулируют!", `GameReactions.cs:312-317`). Attack losses stack per enemy; his next attacks on them use fight-skill ×2/×4/×8/×16 by stack (reset on win) (`CP:1293-1336, 1980-2000`).
- **Буль** — psyche <7: skip chance 1/(10+5×psyche) per round (`CP:4886-4899`); at psyche 0: +2 Str +22 Skill until psyche rises (`CP:4902-4915`).
- **Испанец** — on a loss: 50% (guaranteed every 2nd) +10 Skill −1 Psyche (`CP:2657-2681`); any Вред on mylorik instead gives him +1 Мораль (`CharacterClass.cs:213-221`).
- **Повторяет за myloran** — Int starts at 0 (`CP:82-84`); round 5: +4 Int; round 10: +228 Skill (`CP:4919-4932`).
- **Тупорылая Акула** — at psyche 10 (humans only): transforms into Братишка (name + passives swap properly) (`CP:6097-6104`).
...
- **Спящее хуйло** — 2(+up to 2) scheduled sleep rounds in 1–9: forced skip, −30 Skill, sleepy avatar (`CP:5149-5175`); wakes if he chose an attack (the sleep clears, 33% "POSTAV ROLI" joke) (`CP:2685-2698`).
...
- **Еврей** — when anyone else wins a fight vs a target LeCrisp also attacked this turn: the winner's +1 is suppressed and LeCrisp takes it (+1 regular) (`HandleJews`, `CP:6688-6766`); fellow "Еврей"/Napoleon holders are immune victims. Exclusive with Толя at roll time (`StartGameLogic.cs:180-194`). Moral→skill exchange gains the 7→+40 tier.
- **Булинг** — implemented by name checks: DeepList's Стёб doesn't take his psyche/justice; he never steals from DeepList (D4).
- **Гребанные ассассины** — attacker with Str ≥ his+3 (and not blocked/skipped, and not a first-fight DeepList): LeCrisp auto-loses (`CP:485-504`); every attacker who *wasn't* such an assassin gives +1 Psyche **for the next round only** (buffered, previous bonus removed each round) (`CP:776-784, 3565-3575`).
- **Импакт** — rounds without a defensive loss: ImpactTimes++, +1 bonus, +1 next-round Justice (`CP:3577-3593`, loss detection `CP:807-815`); wins: +（ImpactTimes+1) Мораль (`CP:2701-2710`). Also fixes his Speed-quality: resist 6 / kite 2 (`CharacterClass.cs:396-399, 434-442`).
...
- **Обучение** — losing an attack she initiated (with no active training) starts training on the winner's highest stat she's below (`CP:1891-1941`); +1 to that stat per round (`CP:3705-3757`) and on level-ups (`GameReactions.cs:1249-1297`); reaching the target: +3 Мораль +10% Skill, training clears (target can't change until done).
- **Заводить друзей** — first attack per unique enemy: friend +1 regular (`CP:1237-1242`).
- **Го играть** — implemented *inside* Заводить друзей (D4): attacking a befriended blocker/skipper sets Armor/SkipBreak (`CP:1223-1234`).
- **Дракон** — entering round 10: all stats 10, bonus points = Skill/10 − 1 per friend below her (`CP:5378-5409`). Counterplay: Спартанец's DragonSlayer (round-10 auto-win unless a Суппорт buffed her — `CP:1141-1159`).

## Злой Школьник — Tier 6, Int 9 / Str 9 / Speed 9 / Psyche 9 (file `Mitsuki.cs`)

- **Дерзкая школота** — +100 Skill at start (`CP:185-191`); every non-skip round: −20 Skill and two random stats −1 (can hit the same stat) (`CP:3901-3941`); round 1 bonus: +1 regular (`CP:4960-4968`).
- **Много выебывается** — starts at place 1 (`CP:200-208`); +1 regular per round at #1 (`CP:5926-5933`) and per round nobody attacked him (`CP:3943-3960`); +40 Skill for beating his Мишень target (`CP:1667-1675`); Вред from a higher-skill enemy while #1 → self-Drop (`CharacterClass.cs:223-229`).
- **Запах мусора** — logs every attacker (`CP:555-565`); after round 10: −5 bonus to everyone who attacked him ≥2 times (`CP:5935-5958`).
- **Школьник** — one scheduled round in 2–9: forced skip (brother takes the PC) + 5 next-round Justice (`CP:4970-4986`).
...
- **Булькает** — +1 live Justice per round below #1 (`CP:5858-5861`); cannot gain Skill or Мораль at all (`CharacterClass.cs:963, 1010, 1125-1130`) — moral buttons useless by design.
- **Лежит на дне** — +1 regular whenever a leaderboard neighbor loses a fight (`HandleShark`, `CP:6668-6686`).
- **Челюсти** — +1 Speed per unique enemy beaten (`CP:2838+`) and per new place visited (`CP:5901-5912`).
...
- **Им это не понравится** — 2 marks at start (Rick forced; Глеб/mylorik/Спартанец excluded; Школьник only from round 4, Вампур only before round 4), re-rolled on rounds 2/4/6/8 (`CP:109-157, 3852-3897`); wins vs marked: +1 regular +1 bonus (`CP:1864+`); marked blockers: armor-break + auto-win (`CP:1128-1137`).
- **Это привилегия - умереть от моей руки** — every win from round 5: the victim gains **+1 extra** next-round Justice and Спартанец loses **1 Int** (`CP:2876-2884`); additionally, vs top-3 victims his Harm deals extra Str-resist damage scaling with the skill ratio (extra Drops; "THIS. IS. SPARTA!") (`CharacterClass.cs:237-277`).
- **Первая кровь** — ×2 Skill all game (`SetAnySkillMultiplier(1)`, `CP:105-107`); the first attack's winner (him or the target) gets +1 Speed (`CP:1163-1166` + win handling).
- **Они позорят военное искусство** — first attack per unique enemy: −1 Str −1 Speed; mylorik and Кратос are spared and instead both sides gain +1 Psyche (`CP:1168-1193`).
- **DragonSlayer** — round-10 auto-win vs the Дракон unless Суппорт-buffed (`CP:1141-1159`). **Skill 228** — skill display/value capped at 228 (`CharacterClass.cs:902-907`). **Искусство** — flavor intro line (shared with mylorik, `CP:77-80`). **2kxaoc** — hidden; its only special handling is being *exempt* from the enemy-passive-name masking in the stats display, so the meme stays readable (`GameUpdateMess.cs:805-818`, D5).
...
- ⚠ "Вампур_" garlic Easter egg dead (m1); "Vampyr Позорный" disabled (m2); "Vampyr" hidden passive = round-1 intro phrase (`CP:5413-5421`).

## Краборак — Tier 4, Int 2 / Str 3 / Speed 1 / Psyche 9 (file `CraboRack.cs`)

- **Панцирь** — first attack from each unique enemy: auto-block (+3 Мораль +33 Skill), block flag cleared after the interaction (`CP:427-439, 2537-2545`).
- **Болевой порог** — each incoming next-round Justice point: 50% converted to +1 regular instead (`CharacterClass.cs:1654-1672`).
- **Хождение боком** — attackers fight with Speed 0 (`CP:441-443`); 3 scheduled rounds: own Speed →10, restored next round (`CP:5233-5258, 3551-3562`).
- **Питается водорослями** — attacking places 4–6: +1 bonus (`CP:1339-1340`).
- Boole Family: Вред-immune. `BokoBoole` class is dead code (m6).

## Weedwick — Tier 6, Int 3 / Str 3 / Speed 3 / Psyche 6 (file `WeedWick.cs`)

- **Оборотень** — swaps Strength with the target for the fight — attack only (defense case commented out) (`CP:1100-1110, 403-414`).
- **Безжалостный охотник** — always Armor/SkipBreak (`CP:1112-1116`); vs 0-Justice targets (or Rick): Speed ×2 for the fight (`CP:1118-1124`).
- **Ценная добыча** — win pays the victim's win-streak: regular if they're above him, bonus if below (`CP:1766-1780`); extra Вред rolls: 1/place, 1/5, +1/3 vs #1, range ignored (`CP:1811-1859`).
- **Weed** — every player's every win adds a Weed stack on themselves (`DoomsdayMachine.cs:64-66`); Weedwick's win harvests the victim's stacks as Мораль (`CP:1717-1762`); a round without smoking: −1 Psyche (`CP:5848-5854`).
...
- **Одна из трех** (hidden) — game end: if top-3 and alive, she's declared the winner (narrative + ZBS only on score tie; place-based payouts unchanged — D3) (`CheckIfReady.cs:496-508`).
...
- **AutoWin** — targets can't win; Armor/SkipBreak (`CP:993-997`).
...
- **Exploit** — one rotating exploitable player per round (`GameClass.cs:136-170`); every loss while exploitable accrues to a global pot (`DoomsdayMachine.cs:73-76`); Баг's attack on the current carrier patches it (them permanently) and claims the pot as regular points (`CP:1652-1664`).
