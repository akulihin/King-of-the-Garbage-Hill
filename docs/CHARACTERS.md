...
- **Клоны Сусано** — on round 8 Madara is ready/locked and cannot submit an attack. Every live, non-skipping strict bot immediately commits its ordinary attack to Madara before the human readiness gate; a Madara-only human game therefore does not wait out the turn timer (`Madara.cs:208-230`; `CheckIfReady.cs:1028,1155-1157`; `BotsBehavior.cs:94-105`). Each enemy with a locked exact Madara prediction gets one additional queued attack on him; it is intentionally not deduplicated from a normal/hidden/second action, so every clone fight remains visible in the standard fight log. If more than two unique enemies target him, he immediately gains +1 live Justice (`CheckIfReady.cs:1398-1420`; `Madara.cs:64-110,197-230`). The supplied global challenge and theme are sent to every player at opening round 8; web playback is removed when the fights finish (`CP:4919-4938`; `DoomsdayMachine.cs:189-197,1419-1426`). Round 9 selects the supplied dialogue from round-8 unique attackers and resolved defense results. All five unique attackers plus at least five Madara losses seals him: he cannot act or be targeted and the hidden ending is cancelled (`Madara.cs:113-230`; `GameReactions.cs:677-702`).
