...
Fifteen character-story achievements observe existing mechanics for TheBoys, Стая Гоблинов, Рик Санчез, Сайтама, Мадара, Тигр, Итачи, Кратос, Кира, Монстр без имени, Геральт, Котики, Darksci, Эрен Йегер and DooM Guy (`AchievementClass.cs:232-295,458-526`). Seven additional secret achievements observe specific character pairings (`AchievementClass.cs:297-346,528-559`; cross-reference [INTERACTION-MATRIX.md](INTERACTION-MATRIX.md) §8). These hooks do not change the passive rules documented below; they only record match-local facts and evaluate them at game end. Exact requirements, rewards, secret/privacy behavior and the V1 reset are in [ACHIEVEMENTS.md](ACHIEVEMENTS.md).
...
- **Пацаны** — level-up gives **+2** to the chosen stat and levels the matching member: Int→Francie, Str→Butcher, Speed→Kimiko, Psyche→M.M. (`GameReactions.cs:968-1057`). First upgrade appends the supplied upgrade line to that member's description; 4th unlocks the ultimate. Once СуперМудень is active, later level-up points are consumed without reaching the other members (`GameReactions.cs:969-974`).
- **Francie** — first order at game start (`CP:314-330`), replacements on rounds 4 and 7, 3-turn windows; completion = +1 bonus, expiry (rounds 4/7/10) = −1 bonus (`CP:3405-3422,5914-5943`). A final-turn win is settled in the per-fight hook and clears `OrderTarget` **before** the round-transition expiry/new-target check; the display counter cannot veto it (`CP:3410-3420,5918-5927`). Хим.оружие (level ≥1): eligible win pays **`chemLevel × difficulty`** bonus, difficulty = 1 + enemy-TooGood + enemy-TooStronk (`CP:3423-3437`; m10 fixed).
- **Butcher** — marks begin on round 2 after sorting: all permanent sups +2 rotating marks, prioritizing current Мишень class and excluding the active Francie order while Francie exists (`CP:6181-6215`). A resolved attack on a marked sup gives +10 Skill (+20 СуперМудень), win or loss (`CP:3464-3483`). Every actual Drop of that sup gives **+1 regular point** (therefore round-multiplied), or **+2 regular per Drop** under СуперМудень (`DoomsdayMachine.cs:996-1006`; M7 corrected). Кочерга uses fight-Skill multiplier `1 + pokerCount` and repeats Harm `1 + butcherState.PokerCount` times (`CP:1561-1571`, `DoomsdayMachine.cs:929-964`).
- **Kimiko** — her base +10 defense-win / +20 successful-block Skill and Harm guard exist at level 0 (`CP:793-816`, `CharacterClass.cs:217-228`). **Regeneration does not exist mechanically before x1**: Regen 0 cannot arm `DisabledNextRound`, become disabled, recover, or emit those phrases (`CP:951-961,5947-5972`). From x1, defense ignores up to RegenLevel attacker Justice for that fight; a defensive loss disables regeneration/guard for the next round, then it recovers (`CP:704-723,951-961,5947-5972`). **Живое Оружие** (x4): never disabled; after every defensive fight/block it drains the attacker's real Justice, transfers it to TheBoys, and grants the same number of **regular points** (`CP:802-815`).
- **M.M.** — base team psyche: +1 if zero losses this round, −1 + психует if every resolved fight was a loss (`CP:3612-3649`). **First Psyche upgrade immediately makes M.M. calm** (immune to all `MinusPsycheLog`) and arms Компромат (`GameReactions.cs:1021-1029`, `GamePlayerBridgeClass.cs:102-116`); calmness suppresses only the −1/psych-out, while the zero-loss +1 team Psyche continues. The next actual attacking fight files the target with one of the supplied character/position-specific hints (`CP:3486-3514,7176-7308`); round 8 all-correct files pay +5 Мораль each, and game-end correct-prediction bonus is ×kompromat count (`CP:3631-3645`, `CheckIfReady.cs:320-350`). **Оковы Правосудия** (x4) steals current Мораль from every filed target and blocks their later Moral gains; calmness is no longer part of this ultimate (`GameReactions.cs:1030-1050`).
- **Смертельный вирус** (Francie x4): next real attack infects; spreads on later real fights (block/skip excluded; Francie immune); game end steals 2 bonus per infected (`CP:2332-2369,3441-3460`, `CheckIfReady.cs:353-375`).
- **СуперМудень** (Butcher x4) — disables **every** remaining Francie/Kimiko/M.M. path, including seeded-virus spread/end payout, final компромат, calm immunity, recovery phrases and the stale order exclusion (`CP:2332-2369,3398-3403,3488-3492,3616-3649,5914-5972`; `CheckIfReady.cs:320-375`; `GamePlayerBridgeClass.cs:102-116`). It doubles the complete Butcher results: Кочерга #4 fight Skill ×5→×10 and base Harm 5→10 (`CP:1561-1571`, `DoomsdayMachine.cs:929-964`). Every Int/Str/Psyche pool break that actually applies its negative effect queues another Harm; those Harms recurse. It bypasses enemy Harm interceptors (including Madara, Boole Family, Kimiko and Испанец), allows last-place score Drops, stops at victim score 0, and caps at **50 actual Drops shared across the turn** before resetting next turn (`CharacterClass.cs:182-345`; `DoomsdayMachine.cs:939-984`; `CP:6181-6184`; M17 fixed).
- Bots/Discord/web: bots understand the immunity bypass/full doubled Harm (`BotsBehavior.cs:1046-1072`); the owner alone sees 🦸 on marked enemy rows (web `GameStateMapper.cs:137-145,345-351` / `PlayerCard.vue:738-741`; Discord `GameUpdateMess.cs:697-719`). Targets receive no sup notification and the owner widget no longer lists their nicknames (m27 fixed).
...
- **Огурчик Рик** — Block converts to pickle for 2 turns. While active he always clears Block/Skip, accepts every incoming fight and **wins it**, including against later attacker-side win overrides (`DoomsdayMachine.cs:261-278,486-499`; M18). Player keeps control: skip button shown, may level up or fire the Portal Gun (choosing a target auto-confirms the skip) (`CP:5723-5743`, `GameReactions.cs:701-704`, `CheckIfReady.cs:1155-1161` excludes pickle from auto-move). If never attacked while pickled: +1 extra forced-skip penalty turn (`CP:4270-4277`).
...
- **Воскрешенное тело** — no level-up points, persistent Skill/Moral or predictions; no Skill target; every negative persistent or one-fight stat mutation is rejected; `MinusPsycheLog` and ordinary Harm do nothing (`CharacterClass.cs:205-210,781-846,911-1167,1239-1605`; `GamePlayerBridgeClass.cs:102-108`; `DoomsdayMachine.cs:1493-1510`). **СуперМудень is the explicit Harm exception** and bypasses this interceptor (`CharacterClass.cs:205-210`). Madara deals no Harm when attacking (`DoomsdayMachine.cs:831-935`). He is immune to every external kill source: Death Note, Kratos, Rumbling and Monster pawn execution (`CP:1725-1734,3527-3541,4287-4300,4583-4594`).
...
- **Клоны Сусано** — on round 8 Madara is ready/locked and cannot submit or reopen any action. His human prediction-confirm state is preserved after the round transition and reasserted before every readiness count, so he never waits on a prediction control he does not have (`CheckIfReady.cs:1463-1487`; `Madara.cs:197-230`). Every live, non-skipping strict bot first spends all pending level-up points, then immediately commits its ordinary attack to Madara before the human readiness gate; a Madara-only human game therefore advances immediately (`BotsBehavior.cs:72-132`; `CheckIfReady.cs:1027-1031`). Each enemy with a locked exact Madara prediction gets one additional queued attack on him; it is intentionally not deduplicated from a normal/hidden/second action, so every clone fight remains visible in the standard fight log. If more than two unique enemies target him, he immediately gains +1 live Justice (`CheckIfReady.cs:1398-1420`; `Madara.cs:64-110,197-230`). The supplied global challenge and theme are sent to every player at opening round 8; web playback resolves the `/sound/...` media URL to the canonical R2 CDN file and is removed when the fights finish (`CP:4944-4956`; `sound.ts:170-180`; `MediaMessages.vue:90-130`; `DoomsdayMachine.cs:189-197,1408-1413`). Round 9 selects the supplied dialogue from round-8 unique attackers and resolved defense results. All five unique attackers plus at least five Madara losses seals him: he cannot act or be targeted and the hidden ending is cancelled (`Madara.cs:113-230`; `GameReactions.cs:677-702`).
- **Вечное Цукуеми** — hidden from every passive list, including Madara's. It arms when all five unique enemies target Madara in one turn, or Madara enters round 10 at place 1; sealing cancels it (`Madara.cs:47-110,189-194`; `CP:6126-6143`; `GameStateMapper.cs:980-993`). After authoritative round-10 settlement/rewards, each non-Madara viewer receives a final-only projection: that viewer is alive, place 1, won a fight, and has exactly the bonus needed to beat the best living score, sourced as `Вечное Цукуеми`. If that character has a game-win theme, only their own theme plays locally; no authoritative winner theme leaks into the illusion (`Game.vue:233-274`; `sound.ts:974-983`). Madara alone sees the real winner and five skips; spectators receive no result data. The authoritative `GameClass`, account payouts and history are never mutated (`Madara.cs:219-259`; `GameStateMapper.cs:998-1123`; `GameUpdateMess.cs:163-188,1106-1112`). Because a shared artifact cannot represent six incompatible endings safely, activated games do not save a replay or generate an AI story (`CheckIfReady.cs:795-807`; `GameNotificationService.cs:78-88`).
...
- **Медитация** — his skip becomes block (`DoomsdayMachine.cs:238-253`, `CheckIfReady.cs:1163-1168`); blocking applies oils and reveals one enemy's monster hint. Human hints are generated once as paired `<ru>`/`<en>` adaptations; malformed/unavailable AI output falls back to the paired static hint, and either path is stored in a replay-safe bilingual phrase payload (`ClaudeHaikuService.cs:34-108`; `CP:4661-4713`). **10%** one-time Lambert fumble (m16 fixed): loses **all** Skill for the next round (`CP:4715-4722,746-751,1683-1689`). Геральт gains no Мораль at all (`CharacterClass.cs:1132-1134`).
...
- **Чеканная монета** (hidden demand economy — **web + bot only, no Discord UI**) — in the post-round ready phase the web demand panel (`PlayerCard.vue:960-1004` → hub `DemandContractReward` → `WebGameService.cs:577-663`) offers two billings off an **invoice** of the previous round's per-target contract fights (`ContractDemandClass.CalculateInvoice`, `Geralt.cs:165-290` — scores wins/losses, target positions, Lambert/meditation penalties and reputation into a 0-2 **coin** / 0-3 **Displeasure** tier; snapshot at `CP:4429-4452`):
...
- **Испанец** — on a loss: 50% (guaranteed every 2nd) +10 Skill −1 Psyche (`CP:2575-2599`); ordinary Вред on mylorik instead gives him +1 Мораль, but СуперМудень bypasses the conversion (`CharacterClass.cs:231-238`).
...
- **Rune** — **Вознесение** immediately gives +8 Int; lost interactions can remove at most those 8 granted points. **Маневры** gives +5 Speed; received Harm can remove at most those 5 granted points. Separate remaining-grant counters prevent either penalty from consuming base or later externally earned stats (`DoomGuy.cs` `ApplySelectedModule`; `CP:2587-2632`; `CharacterClass.cs:213-221`). Reward **Истребление** records unique defeated enemies; after all five, +1 to every stat and bonus points equal to `max(0, 10 − current round)`. Melee reward **Glory kill** uses ×2 fight Skill against either immediate leaderboard neighbour and a win against that neighbour grants +1 to all four persistent stats (`DoomGuy.cs` `ApplyFightModules`; `CP:2587-2632`).
- **Shield** — **Щит-пила** makes an attacker stopped by DooM Guy's block lose 3 bonus points instead of 1. **Шоковый щит** makes the first attacker stopped by the block automatically submit a skip on the next round; the charge is match-once, and bot automation respects the forced skip instead of replacing it with an attack (`DoomsdayMachine.cs:584-613`; `CP:5062-5085`; `BotsBehavior.cs:84-93`). Reward **Адский блок** grants +666 Extra Skill once when at least two attacks hit the same block in one round. **Контр-атака** marks every attacker stopped by the block for its next turn; in every attack or defense against DooM Guy during that turn, the marked enemy's fight Skill and Justice are forced to 0 (`CP:771-788`; `DoomGuy.cs` `ApplyFightModules`). **Щит-акула** converts DooM Guy's submitted block into a non-attacking but fightable stance for that round and temporarily adds Братишка's complete **Ничего не понимает** passive, including Int 0 in every incoming fight and the per-attacker first-contact −1 persistent Int (`DoomsdayMachine.cs:240-249`; `DoomGuy.cs` `PrepareSharkShield`; `CP:503-513`).
- **Mission** — **Адеские гнезда** places one nest on a new living enemy when selected and at each next-round setup; only an attacking **win** against that carrier destroys it for +1 regular point. A loss, block or skip leaves the nest. More than three active nests causes −20 immediate bonus and clears the outbreak. **Навести беспорядок** grants +1 regular for every resolved fight, win or loss (`DoomGuy.cs` `SpawnDemonNest`; `CP:2636-2652`). Reward **Стань богом** gives +20 bonus after round 10 if DooM Guy never blocked and never lost after the module became active. **Ближник** doubles the hidden bonus portions of melee modules against immediate leaderboard neighbours: Кулаки pay +4 regular instead of +2; Glory kill uses ×3 total Skill and grants +2 of every stat; Бензопила permits two distinct selections from the same up-to-four choices instead of one (`CP:2620-2685`; `DoomGuy.cs` `ApplyFightModules`, `CopyChainsawPassive`). The melee classification (Glory kill, Кулаки, Бензопила) and these amplifiers are design-only metadata and are not sent as extra player-facing module text.
- **Gun** — **BFG** starts charged. It is preserved by blocking and by decisive fights; on the next attack that reaches Step 3 random, it is spent and makes that primary result a win. A winning primary launches two leaderboard-neighbour branches. Every secondary wave fight that reaches Step 3 also destroys randomness and becomes a win; a decisive pre-random loss, block or skip still ends that branch (`DoomsdayMachine.cs:810-881`). **Кулаки** sets persistent Strength to 0 and gives +2 regular on each win. Reward **Бензопила** is spent on the next win, offers up to the first four non-admin passives of the victim, replaces Gun with the selected passive(s), and primes one use for the explicit charge passives Portal Gun, Шэн, Изанаги and Глаза Итачи (`CP:2656-2685`; `DoomGuy.cs` `CopyChainsawPassive`). **Рельса** starts with one charge; the next submitted attack spends it and queues the selected target plus every living, non-team, non-sealed enemy on that side of DooM Guy. Those Railgun fights bypass each target's Block and Skip; the round-10 Тигр ban is excluded from the fan-out (`DoomsdayMachine.cs:445-491,569-650`). Special **Приручить дракона** adds the hidden **Дракон** passive and changes Geralt's target classification to Драконы: on entering round 10 DooM Guy receives Sirinoks's exact transformation (all stats 10, `Skill / 10 − friends below` bonus) and therefore every passive-name interaction, including DragonSlayer and dragon threat display (`DoomGuy.cs` `ApplySelectedModule`; `CP:5767-5797`).
...
- **Дрочун** — whenever an enemy starts an attack against Eren, its mark becomes 2 immediately; an Eren loss sets the winner's mark to at least 1 and never downgrades 2 (`CP:470-473, 2523-2534`). An Eren victory cashes only the defeated enemy's current mark as that many immediate bonus points and clears it (`CP:2536-2545`). If Eren's own attack target also targeted Eren that turn, Eren gets +2 regular points once per enemy per round, regardless of the result; the server increments an audio serial for `eren_tatake.mp3` (`CP:2547-2556`; `ErenYeager.cs:21-23`; `Game.vue:211-216`; `sound.ts:986-988`). Marks are shown as 🔥1/2 in Discord/web leaderboards and listed in the owner widget (`GameUpdateMess.cs:459-461`; `GameStateMapper.cs:396-400`).
- **Атакующий Титан** — during calculation, Eren's block flag is removed before block resolution, so he never blocks attacks; the Titan state/sound serial is armed (`DoomsdayMachine.cs:271-281`). Each fight that round reapplies exact +5 temporary Int/Str/Speed/Psyche overrides on the `FightCharacter`, so all incoming/forced fights receive the boost and values may exceed the normal persistent 10 cap (`CP:62-72, 475-479, 1048-1052`). If nobody targeted Eren that turn, end-of-round processing removes 2 Psyche through `MinusPsycheLog`; otherwise there is no penalty (`CP:3599-3620`). The client rolls `eren_attack_on_titan_use_most` at 50%; the other 50% is uniform over files 1–3 (`sound.ts:990-995`).
- **Rumbling** — only resolved losses during **round 10** increment the gate counter; losses from rounds 1–9 do not matter (`CP:2563-2568`). Opening round 10 writes the supplied Armin warning verbatim (`CP:4990-5000`) and starts both rumbling audio files together for every connected web player (`Game.vue:218-228`; `sound.ts:997-1000`). Immediately after all round-10 fights, before ordinary end-of-round passives, a living Eren with fewer than two round-10 losses projects the leaderboard using current score plus the pending round score with its real multiplier (including `Подсчет`) (`DoomsdayMachine.cs:1419-1420`; `ErenYeager.cs:37-71`; `CP:3548-3563`). Every living player strictly between Eren's projected place and place 6 dies with source `Rumbling`; place 6 (Eldia) itself survives. Goblins have no immunity; Shisui's general next-round revive still applies. Deaths feed Монстр's +1-per-death passive before later end-of-round passives execute (`CP:3565-3593`).
- **Audio/UI** — owner widget exposes gained Rage, losses toward the Rumbling gate, Titan state, hatred marks and resolved Rumbling place (`GameStateDto.cs:775-793`; `GameStateMapper.cs:349-370`; `PlayerCard.vue:1130-1147`). Tatake/Titan sounds are owner-only serial events; the Rumbling warning and `eren_game_win_theme.mp3` are global (`Game.vue:180-228`; `sound.ts:968-1000`).
