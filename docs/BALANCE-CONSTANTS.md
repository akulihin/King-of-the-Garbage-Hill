...
|---|---|---|---|
| Homelander | base stats / Tier | Int 5, Str 10, Speed 9, Psyche 1; Tier 3 (roll weight 80, human-only seats, no Discord Auto Move button) | `characters.json` Homelander |
| Homelander | Праведность leader payout / Seven score / rage trigger / cap | place 1: +7 Moral per turn and virtual +7 score; otherwise +25% rage on the leader; any enemy victory over Homelander +10%, or +50% when that enemy was charged as leader in the same turn (total +75%); cap 100% | `Homelander.ApplyRighteousness`/`RecordEnemyVictory`/`UpdateSevenPointsAvailability`; `InGameStatusClass.GetScore`; `LeaderMoral`/`SevenPoints`/`LeaderRage`/`VictoryRage`/`LeaderVictoryRage`/`MaximumRage` |
