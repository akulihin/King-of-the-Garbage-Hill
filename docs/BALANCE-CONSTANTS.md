...
| DualSense input hysteresis / gates | activation 0.22; release 0.14; hysteresis 0.08; shallow/medium/deep/final gates 0.22/0.48/0.72/0.90 | `public/99lc/game-config.json` `input.dualsense`; `features/last-chances/control-schemes.ts` `DualSenseControlRecognizer` |
...
| DualSense click/ramp/band profiles | each tuple is start–end / resistance / force / transition ms / effect ms / magnitude: click .18–.30/.24/.28/35/90/.22; ramp .22–.86/.30/.55/140/600/.38; light band .20–.38/.22/.25/30/80/.20; medium band .40–.62/.34/.42/35/95/.34; strong band .64–.84/.48/.58/40/110/.48 | `public/99lc/game-config.json` `input.dualsense.feedback.profiles`; `features/last-chances/feedback.ts` |
| DualSense gate/follow-up/blocked profiles | gate .48–.78/.52/.62/70/320/.46; follow-up .30–.56/.30/.36/45/140/.30; blocked .12–.34/.46/.42/45/160/.32 | `public/99lc/game-config.json` `input.dualsense.feedback.profiles`; `features/last-chances/feedback.ts` |
| DualSense impact/tension profiles | impact .16–.44/.38/.50/25/120/.55; tension .26–.82/.44/.58/100/520/.40; a combo node may override any safe profile field | `public/99lc/game-config.json` `input.dualsense.feedback.profiles` and `weapons[*].controls.*.dualsense.nodes[*].adaptiveOverride`; `features/last-chances/feedback.ts` |
| Seven-weapon catalog / Chance prices | Spear 3; Chain 2; Claws 2; capture-only Knife-spider 0; Axe 3; Katana 3; Меч наемника 2 | `public/99lc/game-config.json` `weapons[*].chanceCost`; `rooms[*].interaction` |
...
| Меч наемника rhythm / stagger / execution | ideal rhythm starts at 500 ms and remains perfect through 600 ms; early input causes 2,000 ms Sword-only fatigue; each accepted Zornhaw advances 14.4 units at 144 units/s; matching 160 px mouse travel grants up to +25% damage; empty off-hand multiplies all Sword damage ×1.5; Zornhaw staggers 500 ms; elite/boss targets become Unstoppable for 5,000 ms after 3,000 ms accumulated stagger; opening every 3 basic hits for 1,600 ms; Oberhaw 26 base damage, ×0.55 without opening or ×3.25 with opening, 2,800 ms cooldown and 48-unit collider width; Unterhaw hold gate 1,000 ms | `public/99lc/game-config.json` `hybrid-sword`; `features/last-chances/engine.ts` `applySwordRhythm`/`applySwordStagger`/`damageEnemy`/`executePendingUnterhau` |
...
| Гордон Фримен | Монтировка | every 3rd resolved attack-or-defense fight is a terminal Gordon win; no H.E.V. Justice-to-stat conversion | `GordonFreeman.BeginResolvedFight`; DM crowbar outcome override |
