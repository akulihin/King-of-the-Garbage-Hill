...
> Hand-maintained. **Update the row when you change the number** (and re-run `tools/audit-passives.sh` for name changes). Verified against the working tree of 2026-07-29 (v5.1.53). `CP` = `Game/GameLogic/CharacterPassives.cs`, `CC` = `Game/Classes/CharacterClass.cs`, `DM` = `Game/GameLogic/DoomsdayMachine.cs`, `CIR` = `Game/GameLogic/CheckIfReady.cs`, `GR` = `Game/ReactionHandling/GameReactions.cs`.
>
...
| Мадара | Второй метеорит | attacking win: enemy current Moral →0 through ordinary protection; blocked attack: no −1 bonus, +2 regular, blocker's real Justice set to 0 instead of the ordinary +1 block grant (outranks Близнец) | `Madara.ApplySecondMeteoriteMoral`; DM `CalculateAllFights` win/block paths |
...
| Джон Сноу | Я Джон Сноу | current Justice added to all 4 fight stats; the same pre-hook snapshot gives toogood +1 Justice/stats or toostronk +2 Justice/stats, so in-fight stat gains cannot change the tier | `JonSnow.CaptureDifficulty`/`ApplyBaseJustice`/`ApplyDifficultyJustice` |
