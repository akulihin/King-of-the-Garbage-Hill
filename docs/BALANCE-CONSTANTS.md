...
> Hand-maintained. **Update the row when you change the number** (and re-run `tools/audit-passives.sh` for name changes). Verified against the working tree of 2026-07-30 (v5.2.3). `CP` = `Game/GameLogic/CharacterPassives.cs`, `CC` = `Game/Classes/CharacterClass.cs`, `DM` = `Game/GameLogic/DoomsdayMachine.cs`, `CIR` = `Game/GameLogic/CheckIfReady.cs`, `GR` = `Game/ReactionHandling/GameReactions.cs`.
>
...
The complete browser-prototype definition remains externally editable in `Web/VueClient/public/99lc/game-config.json`; schema v11 keeps the weapon catalog, all three control schemes, the stamina block, every collider/trace and every behavior-specific number in that no-rebuild definition. Its v10→v11 migration upserts only the canonical `twohand-bow` record, preserving the user's loadout, catalog order and every unrelated weapon. The Builder exposes the control/timing/feedback values below and preserves bindings, per-node overrides and remaining `tuning` keys through raw JSON import/export.
...
| Двуручное копьё v2 «Замах» / «Акали» bands | Both use the v2 325/563/825 ms thresholds. «Замах»: «Заколоть» (×2.2 damage, ×0.14 range, radius 34, 70°, no shaft guard) / throw-and-stun / piercing wall-pin at 10/15/20 stamina. «Акали» has a `spin-early` band at 325 ms carrying the wide rassekatel (×1.25 damage, ×1.28 range, 165°, single hit) ahead of the two overhead spins, at 15/20/25 stamina. The DualSense follow-up gate uses `requiredChargeBandId: 'early'` to match | `public/99lc/game-config.json` `twohand-spear-v2.attacks.hold`/`holdThenDoubleTap` charge bands and `controls.primary`; `features/last-chances/config.ts` `ATTACK_SET_CONTROL_SEEDS`; `features/last-chances/engine.ts` `performSpearReleaseV2`/`performSpearOverheadSpin` |
| Двуручное копьё v2 secondary geometry / force | «Парирование» is a 180-unit transverse bar with 28-unit thickness, centred 36 units forward. Ordinary «Толчок» places a target 176 units out (×2 from 88); the early charged «Толчок» uses 188 (×2 from 94). Both deeper releases are named «Пинок» and place targets at 432 / 528 units (×3 from 144 / 176), with 1,000 / 1,500 ms stun. The held second press still grants ×2 armor only while down | `public/99lc/game-config.json` `twohand-spear-v2.secondaryAttacks`; `features/last-chances/engine.ts` `sidewaysAttackCollider`/`resolveSpearKickOutcome`/`damageEnemy` spear shove/kick branch |
| Двуручное копьё v2 «Строй» / «Олимпийский прыжок» | «Строй» spends 5 stamina / 100 ms, never self-rotates, and uses actual tip-versus-target relative velocity against 420 piercing / 900 cutting reference speeds (×1.8 / ×0.95 above the stationary ×0.3 floor). «Олимпийский прыжок» travels up to 360 units through 180 ms run + 120 ms fixed plant + 180 ms rise + 420 ms flight + 150 ms landing; airborne traversal ignores intermediate obstacles, while boundaries and the final landing footprint remain solid | `public/99lc/game-config.json` `twohand-spear-v2.secondaryAttacks.hold`/`holdThenDoubleTap`; `features/last-chances/engine.ts` `updateActiveAreas`/`spearStanceDamageMultiplier`/`poleVaultTravelDistance`/`updatePoleVault` |
...
| Злая горючка upgrade | +2 coins; replaces that barge's ordinary Горючка with one Злая горючка weapon holding 2 shots | `ShipCatalog.Definitions` `barge_evil_incendiary`; `ShipCatalog.ApplyUpgrade` `evil_incendiary` |
...
| Final Boarding deployment capacity | frozen at the number of enemy row-1 cells not occupied by a living summon when Boarding starts, therefore at most 10 mandatory deployments per player; every deployment consumes one capacity and all overflow is discarded | `BattleshipGameEngine.PrepareMandatoryBoardingSummons`/`DiscardMandatoryBoardingRemainder`; `BattleshipService.CompleteMandatoryBoardingDeployment` |
| Final Boarding ammunition | every operational Tetracatapult +1 of its configured shot; Triple `extra_ammo_boarding` +2 more; every operational Горючка or Злая горючка +1. Each grant raises current ammo and `MaxAmmo` equally | `BattleshipGameEngine.TriggerBoarding` |
...
| Мадара | Вечное Цукуеми | arm at all 5 unique attackers in one turn, **zero round-8 losses (including zero fights)**, or raw score place 1 at the r10 boundary; winner is captured immediately after r9 score settlement and before all r10/final modifiers; fooled-viewer bonus = max living captured score − viewer score + 1 (0 if sole winner) | `Madara.ResolveRoundNine`/`TryCaptureEternalTsukuyomiEnding`/`GetIllusoryBonus`; `DoomsdayMachine.CompleteRoundAsync` |
...
| Гордон Фримен | Просыпайтесь, мистер Фримен | 1 use/game; any living non-Kratos-event Skip or active Глаза Итачи target (wake cancels that target); optional r9 reserve only while Вечное Цукуеми is already armed makes Gordon an authoritative ending viewer, but opens no real r10 action | `GordonFreeman.CanWake`/`Wake`/`CancelItachiEyes`/`SeesEternalTsukuyomiReality`; `Madara.IsEternalTsukuyomiActive`/`TryCaptureEternalTsukuyomiEnding` |
