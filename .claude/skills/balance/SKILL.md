...
3. **Do NOT touch `characters.json` descriptions** — they are deliberately vague player-facing text (CLAUDE.md rule). If the old number is spelled out in the RU description ("+2 очка", "50%"), flag it in your summary so the designer can decide whether to reword; the exact new value goes into `docs/CHARACTERS.md` and `docs/BALANCE-CONSTANTS.md` only.
4. Apply the code change (respect CLAUDE.md correctness rules — e.g. regular vs bonus points changes the round-multiplier behavior).
